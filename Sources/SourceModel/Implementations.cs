...
namespace Microsoft.Cci
{
	...
	public sealed class SemanticErrorReporter : ISemanticErrorsReporter
	{
		...
		/// </summary>
		public static readonly SemanticErrorReporter Instance = new SemanticErrorReporter();
	}
	...
	public abstract class SourceEditHostEnvironment : MetadataReaderHost, ISourceEditHost
	{
		...
		public SourceEditHostEnvironment(INameTable nameTable, IInternFactory factory, byte pointerSize, IEnumerable<string> searchPaths, bool searchInGAC) : base(nameTable, factory, pointerSize, searchPaths, searchInGAC)
		{
		}
		...
		public IUnitSet GetUnitSetFor(UnitSetIdentity referencedUnitSet)
		{
			...
			lock (GlobalLock.LockingObject) {
				if (this.unitSetCache.TryGetValue(referencedUnitSet, out entry)) {
					if (entry != null)
						result = entry.Target as IUnitSet;
				}
			}
		...
		/// </summary>
		public static bool UnitsAreTheSameObjects(List<IUnit> unitList, IEnumerable<IUnit> unitEnumeration)
		{
			var enumerator = unitEnumeration.GetEnumerator();
			for (int i = 0, n = unitList.Count; i < n; i++)
				if (!enumerator.MoveNext() || !object.ReferenceEquals(unitList[i], enumerator.Current))
					return false;
		...
		public void ReportEditsUsingDifferentThread(object state		)
		{
		...
		/// <param name="state">The edit event arguments.</param>
			/*?*/		//^ requires state is EditEventArgs;
		public void ReportSymbolTableEditsUsingDifferentThread(object state		)
		{
			EditEventArgs editEventArguments = (EditEventArgs)state;
			if (this.SymbolTableEdits != null)
	...
	public abstract class SourceDocumentEdit : ISourceDocumentEdit
	{
		...
		public SourceDocumentEdit(ISourceLocation sourceLocationBeforeEdit, ISourceDocument sourceDocumentAfterEdit)
		{
...
//^^ ensures result.IsUpdatedVersionOf(this.SourceLocationBeforeEdit.SourceDocument);
			get { return this.sourceDocumentAfterEdit; }
		}
	...
	public abstract class ErrorMessage : ISourceErrorMessage
	{
		...
		public ErrorMessage(ISourceLocation sourceLocation, long errorCode, string messageKey, params string[] messageArguments)
		{
		...
		/// <param name="messageArguments">Zero or more strings that are to be subsituted for "{i}" sequences in the message string return by GetMessage.</param>
		public ErrorMessage(ISourceLocation sourceLocation, long errorCode, string messageKey, IEnumerable<ILocation> relatedLocations, params string[] messageArguments)
		{
			this.sourceLocation = sourceLocation;
			this.errorCode = errorCode;
			this.messageKey = messageKey;
		...
		/// </summary>
		public abstract object ErrorReporter { get; }
		...
		/// </summary>
		public abstract string ErrorReporterIdentifier { get; }
		...
		public string GetMessage(System.Resources.ResourceManager rm)
		{
			...
			if (localizedString == null)
				localizedString = this.messageKey;
	...
	public abstract class CompositeSourceDocument : SourceDocument, IDerivedSourceDocument
	{
		...
		public CompositeSourceDocument(IName name) : base(name)
		{
		}
		...
		public CompositeSourceDocument(SourceDocument previousVersion, int position, int oldLength, int newLength) : base(previousVersion, position, oldLength, newLength)
		{
		}
		...
		{
			if (this.length != null && position >= (int)this.length)
				return 0;
			...
			while (this.enumeratorIsValid) {
				...
				if (this.currentFragmentOffset + fragmentLength > position)
					break;
			...
			while (this.enumeratorIsValid && charsCopied < length) {
				int fragmentLength = this.fragmentEnumerator.Current.Length;
				int fragmentStart = 0;
				if (position > this.currentFragmentOffset)
					fragmentStart = position - this.currentFragmentOffset;
				...
				if (charsCopied + fragmentCharsToCopy > length)
					fragmentCharsToCopy = length - charsCopied;
			...
			if (!this.enumeratorIsValid)
				this.length = this.currentFragmentOffset;
		...
		/// </summary>
		public abstract IEnumerable<ISourceLocation> GetFragments();
		...
		int? length;
		//^ invariant length == null || (int)length >= 0;
		/// <summary>
		...
		public void ToLineColumn(int position, ref int line, ref int column, string text)
		{
	...
	public class IncludedSourceLocation : IIncludedSourceLocation
	{
		...
		public IncludedSourceLocation(SourceDocumentWithInclusion document, int startIndex, int length)
		{
...
//^^ ensures result == this.StartIndex + this.Length;
		...
		public int OriginalEndLine {
			get { return this.EndLine - this.InclusionStartLine + this.SourceDocument.OriginalLineNumber; }
		}
		...
		public int OriginalStartLine {
			get { return this.StartLine - this.InclusionStartLine + this.SourceDocument.OriginalLineNumber; }
		}
	...
	public abstract class PrimarySourceDocument : SourceDocument, IPrimarySourceDocument
	{
		...
		public PrimarySourceDocument(IName name, string location, StreamReader streamReader) : base(name)
		{
			this.location = location;
			this.GetOwnStreamReaderAndReadAhead(streamReader);
		}

		public void GetOwnStreamReaderAndReadAhead(StreamReader streamReader)
		{
		...
		public PrimarySourceDocument(IName name, string location, string text) : base(name)
		{
			this.location = location;
			this.text = text;
		}
		...
		/// <param name="newLength">The number of replacement characters in the new document. 
		/// (The length of the string that replaces the substring from position to position+length in the previous version of the new document.)</param>
		public PrimarySourceDocument(string text, SourceDocument previousVersion, int position, int oldLength, int newLength) : base(previousVersion, position, oldLength, newLength)
		{
		...
		/// </summary>
		public PrimarySourceDocument(PrimarySourceDocument template) : base(template.Name)
		{
			...
			if (this.text != null) {
				...
				lock (GlobalLock.LockingObject) {
					...
					while (result < length) {
						...
						if (charsToCopy > length - result)
							charsToCopy = length - result;
		...
		public override int Length {
			...
			get {
				...
				if (this.length == null) {
					if (this.text != null)
						result = this.text.Length; else if (this.streamReader != null)
		...
		/// </summary>
		char[] 		/*?*/buffer;
		//^ invariant streamReader != null ==> buffer != null;
		/// <summary>
		...
		public void SetStreamReaderPositionTo(int position)
		{
		...
		public override void ToLineColumn(int position, out int line, out int column)
		{
			...
			while (i < position) {
				switch (text[i++]) {
					...
					case (char)0x2029:
						line++;
						//TODO: Boogie crashes here, presumably because it does not like ++ on an out parameter
						column = 1;
		...
		/// </summary>
		public int FindBestStartPointForPosition(int position)
		{
			int best = 0;
		...
		/// </summary>
		public int FindBestStartPointForLine(int line)
		{
			int best = 0;
			int bestDistance = int.MaxValue;
	...
	public abstract class SourceDocument : ISourceDocument
	{
		...
		/// </summary>
		/// <param name="name">The name of the document. Used to identify the document in user interaction.</param>
		public SourceDocument(IName name)
		{
			this.name = name;
		}
		...
		/// <param name="newLength">The number of replacement characters in the new document. 
		/// (The length of the string that replaces the substring from position to position+length in the previous version of the new document.)</param>
		public SourceDocument(SourceDocument previousVersion, int position, int oldLength, int newLength)
		{
	...
	public sealed class SourceDocumentWithInclusion : IPrimarySourceDocument
	{
		...
		public string OriginalDocumentName {
			get { return this.originalDocumentName; }
		}
		...
		public int OriginalLineNumber {
			get { return this.originalLineNumber; }
		}
...
//^ ensures 0 <= result && result < this.Length;
			get { return this.startingPositionOfIncludedRegion; }
		}
	...
	public abstract class SourceLocation : ISourceLocation
	{
		...
		public SourceLocation(int startIndex, int length)
		{
			this.startIndex = startIndex;
			this.length = length;
		}
		...
		public bool Contains(ISourceLocation location)
		{
			if (location.SourceDocument != this.SourceDocument) {
				...
				foreach (ISourceLocation sloc in cdoc.GetPrimarySourceLocationsFor(dloc)) {
					if (sloc.SourceDocument != location.SourceDocument)
						continue;
		...
		/// The document containing the source text of which this location is a subrange.
		/// </summary>
		public abstract ISourceDocument SourceDocument { get; }
	...
	public class DerivedSourceLocation : SourceLocation, IDerivedSourceLocation
	{
		...
		public DerivedSourceLocation(IDerivedSourceDocument derivedSourceDocument, int startIndex, int length) : base(startIndex, length)
		{
	...
	public class PrimarySourceLocation : SourceLocation, IPrimarySourceLocation
	{
		...
		public PrimarySourceLocation(IPrimarySourceDocument primarySourceDocument, int startIndex, int length) : base(startIndex, length)
		{
	...
	public sealed class SourceLocationSpanningEntirePrimaryDocument : PrimarySourceLocation
	{
		...
		public SourceLocationSpanningEntirePrimaryDocument(IPrimarySourceDocument primarySourceDocument) : base(primarySourceDocument, 0, 0)
		{
		}
	...
	public sealed class SourceLocationSpanningEntireDerivedSourceDocument : DerivedSourceLocation
	{
		...
		public SourceLocationSpanningEntireDerivedSourceDocument(IDerivedSourceDocument derivedSourceDocument) : base(derivedSourceDocument, 0, 0)
		{
		}
	...
	public sealed class EditDescriptor : IEditDescriptor
	{
		...
		public ISourceDocument ModifiedSourceDocument {
			get { return this.modifiedSourceDocument; }
		}
