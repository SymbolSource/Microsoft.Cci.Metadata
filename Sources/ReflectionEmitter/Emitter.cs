...
namespace Microsoft.Cci.ReflectionEmitter
{
	...
	public class DynamicLoader
	{
			...
			/*?*/			/*?*/		public DynamicLoader(ISourceLocationProvider sourceLocationProvider		, ILocalScopeProvider localScopeProvider		)
		{
			...
			this.initializingTraverser = new MetadataTraverser {
				PostorderVisitor = this.emitter,
		...
		Emitter emitter;
		MetadataTraverser initializingTraverser;
		//TODO: need the traverser to traverse private helper types/members
		...
		public AssemblyBuilder AssemblyBuilder {
			get {
				if (this.assemblyBuilder == null)
					this.assemblyBuilder = AppDomain.CurrentDomain.DefineDynamicAssembly(new System.Reflection.AssemblyName("CCI generated dynamic assembly " + this.GetHashCode()), AssemblyBuilderAccess.RunAndCollect);
		...
		public AssemblyBuilder Load(IAssembly assembly)
		{
			...
			if (!(assembly.EntryPoint is Dummy))
				this.AssemblyBuilder.SetEntryPoint((MethodInfo)this.mapper.GetMethod(assembly.EntryPoint));
		...
		public class TypeBuilderAllocater : MetadataTraverser
		{

			public TypeBuilderAllocater(DynamicLoader loader)
			{
				this.loader = loader;
			}
			...
			public override void TraverseChildren(INamespaceTypeDefinition namespaceTypeDefinition)
			{
				...
				this.loader.mapper.DefineMapping(namespaceTypeDefinition, typeBuilder);
				//so that typeBuilder references can be treated uniformly later on
			...
			public override void TraverseChildren(INestedTypeDefinition nestedTypeDefinition)
			{
				...
				if (nestedTypeDefinition.IsGeneric)
					name = name + "`" + nestedTypeDefinition.GenericParameterCount;
				...
				this.loader.builderMap.Add(nestedTypeDefinition, typeBuilder);
				//so that typeBuilder references can be treated uniformly later on
			...
			public void AllocateGenericParametersIfNecessary(ITypeDefinition typeDefinition, TypeBuilder typeBuilder)
			{
			...
			public static TypeAttributes GetTypeAttributes(ITypeDefinition typeDefinition)
			{
				...
				if (typeDefinition.StringFormat == StringFormatKind.Unicode)
					attributes |= TypeAttributes.UnicodeClass;
				if (typeDefinition.StringFormat == StringFormatKind.AutoChar)
			...
			public static TypeAttributes GetNestedTypeVisibility(ITypeDefinitionMember typeDefinitionMember)
			{
		...
		public class MemberBuilderAllocator : MetadataTraverser
		{
			public MemberBuilderAllocator(DynamicLoader loader)
			{
				this.loader = loader;
			}
			...
			public override void TraverseChildren(IFieldDefinition fieldDefinition)
			{
				...
				this.loader.mapper.DefineMapping(fieldDefinition, fieldBuilder);
				//so that all field references can be treated uniformly later.
			}

			public FieldAttributes GetAttributes(IFieldDefinition fieldDefinition)
			{
			...
			public void GetConstraints(IGenericParameter genericParameter, out List<Type> interfaceConstraints, out Type classConstraint)
			{
			...
			public static GenericParameterAttributes GetAttributes(IGenericParameter genericParameter)
			{
				var result = GenericParameterAttributes.None;
				if (genericParameter.Variance == TypeParameterVariance.Covariant)
					result |= GenericParameterAttributes.Covariant; else if (genericParameter.Variance == TypeParameterVariance.Contravariant)
			...
			public override void TraverseChildren(IPropertyDefinition propertyDefinition)
			{
				...
				var propertyBuilder = containingType.DefineProperty(propertyDefinition.Name.Value, attributes, returnType, rtReqMods, rtOptMods, parameterTypes, ptReqMods, ptOptMods);
			...
			public override void TraverseChildren(IMethodDefinition method)
			{
				...
				if (method.IsConstructor)
					builder = containingType.DefineConstructor(attributes, callingConvention, parameterTypes, ptReqMods, ptOptMods); else if (method.IsStaticConstructor)
					builder = containingType.DefineTypeInitializer(); else if (method.IsPlatformInvoke)
					builder = containingType.DefinePInvokeMethod(method.Name.Value, method.PlatformInvokeData.ImportModule.Name.Value, method.PlatformInvokeData.ImportName.Value, attributes, callingConvention, returnType, rtReqMods, rtOptMods, parameterTypes, ptReqMods,
					ptOptMods, GetNativeCallingConvention(method), GetNativeCharset(method));
				else {
					...
					builder = containingType.DefineMethod(method.Name.Value, attributes, callingConvention, returnType, rtReqMods, rtOptMods, parameterTypes, ptReqMods, ptOptMods);
				}
				this.loader.builderMap.Add(method, builder);
				this.loader.mapper.DefineMapping(method, builder);
			}

			public System.Runtime.InteropServices.CallingConvention GetNativeCallingConvention(IMethodDefinition method)
			{
				switch (method.PlatformInvokeData.PInvokeCallingConvention) {
					case PInvokeCallingConvention.CDecl:
						return System.Runtime.InteropServices.CallingConvention.Cdecl;
					case PInvokeCallingConvention.FastCall:
						return System.Runtime.InteropServices.CallingConvention.FastCall;
					case PInvokeCallingConvention.StdCall:
						return System.Runtime.InteropServices.CallingConvention.StdCall;
					case PInvokeCallingConvention.ThisCall:
						return System.Runtime.InteropServices.CallingConvention.ThisCall;
					case PInvokeCallingConvention.WinApi:
						return System.Runtime.InteropServices.CallingConvention.Winapi;
				}
				return 0;
			}

			public System.Runtime.InteropServices.CharSet GetNativeCharset(IMethodDefinition method)
			{
				switch (method.PlatformInvokeData.StringFormat) {
					case StringFormatKind.Ansi:
						return System.Runtime.InteropServices.CharSet.Ansi;
					case StringFormatKind.AutoChar:
						return System.Runtime.InteropServices.CharSet.Auto;
					case StringFormatKind.Unicode:
						return System.Runtime.InteropServices.CharSet.Unicode;
				}
				return System.Runtime.InteropServices.CharSet.None;
			}

			public void GetModifierTypes(IEnumerable<ICustomModifier> customModifiers, out Type[] reqMods, out Type[] optMods)
			{
				reqMods = null;
				optMods = null;
				int reqModCounter = 0;
				...
				if (reqModCounter > 0)
					reqMods = new Type[reqModCounter];
				if (optModCounter > 0)
					optMods = new Type[optModCounter];
				reqModCounter = 0;
				optModCounter = 0;
				foreach (var customModifier in customModifiers) {
			...
			public Type[] GetParameterTypes(ISignature signature, out Type[][] ptReqMods, out Type[][] ptOptMods)
			{
				uint numPars = IteratorHelper.EnumerableCount(signature.Parameters);
			...
			public static MethodAttributes GetMethodAttributes(IMethodDefinition method)
			{
				...
				if (method.IsHiddenBySignature)
					attributes |= MethodAttributes.HideBySig;
				if (method.IsAccessCheckedOnOverride)
			...
			public static CallingConventions GetCallingConvention(CallingConvention callingConvention)
			{
		...
		public class Emitter : MetadataVisitor
		{

			public Emitter(DynamicLoader loader, ISourceLocationProvider sourceLocationProvider, ILocalScopeProvider localScopeProvider)
			{
				this.loader = loader;
				this.sourceLocationProvider = sourceLocationProvider;
				this.localScopeProvider = localScopeProvider;
			...
			public override void Visit(IEventDefinition eventDefinition)
			{
				...
				foreach (var accessor in eventDefinition.Accessors) {
					...
					if (accessor == eventDefinition.Adder)
						eventBuilder.SetAddOnMethod(accessorMethod); else if (accessor == eventDefinition.Remover)
						eventBuilder.SetRemoveOnMethod(accessorMethod); else if (accessor == eventDefinition.Caller)
			...
			public CustomAttributeBuilder GetCustomAttributeBuilder(ICustomAttribute customAttribute)
			{
			...
			public CustomAttributeBuilder GetMarshalAsAttribute(IMarshallingInformation marshallingInformation)
			{
				...
				if (marshallingInformation.UnmanagedType == System.Runtime.InteropServices.UnmanagedType.ByValArray || marshallingInformation.UnmanagedType == System.Runtime.InteropServices.UnmanagedType.LPArray) {
				...
				if (marshallingInformation.UnmanagedType == System.Runtime.InteropServices.UnmanagedType.ByValArray || marshallingInformation.UnmanagedType == System.Runtime.InteropServices.UnmanagedType.ByValTStr || marshallingInformation.UnmanagedType == System.Runtime.InteropServices.UnmanagedType.LPArray) {
					fields.Add(marshalAsAttributeType.GetField("SizeConst"));
					fieldValues.Add((int)marshallingInformation.NumberOfElements);
				}
				if (marshallingInformation.UnmanagedType == System.Runtime.InteropServices.UnmanagedType.LPArray && marshallingInformation.ParamIndex != null) {
			...
			public override void Visit(IMethodDefinition method)
			{
				if (method.IsConstructor || method.IsStaticConstructor) {
					...
					foreach (var parDef in method.Parameters) {
						if (parDef.HasDefaultValue || parDef.IsOptional || parDef.IsOut || parDef.IsMarshalledExplicitly || IteratorHelper.EnumerableIsNotEmpty(parDef.Attributes) || parDef.Name.Value != string.Empty) {
			...
			public MethodImplAttributes GetImplAttributes(IMethodDefinition method)
			{
				MethodImplAttributes result = (MethodImplAttributes)0;
				if (method.IsNativeCode)
					result |= MethodImplAttributes.Native;
				if (method.IsRuntimeImplemented)
					result |= MethodImplAttributes.Runtime;
				if (method.IsUnmanaged)
					result |= MethodImplAttributes.Unmanaged;
				if (method.IsNeverInlined)
					result |= MethodImplAttributes.NoInlining;
				if (method.IsForwardReference)
					result |= MethodImplAttributes.ForwardRef;
				if (method.IsSynchronized)
					result |= MethodImplAttributes.Synchronized;
				if (method.IsNeverOptimized)
					result |= MethodImplAttributes.NoOptimization;
				if (method.PreserveSignature)
					result |= MethodImplAttributes.PreserveSig;
				if (method.IsRuntimeInternal)
					result |= MethodImplAttributes.InternalCall;
				return result;
			}

			public static ParameterAttributes GetAttributes(IParameterDefinition parDef)
			{
			...
			public void Visit(IParameterDefinition parameterDefinition, ParameterBuilder parameterBuilder)
			{
			...
			public static System.Security.Permissions.SecurityAction GetSecurityAction(ISecurityAttribute securityAttribute)
			{
				return (System.Security.Permissions.SecurityAction)securityAttribute.Action;
			}

			public System.Security.PermissionSet GetPermissionSet(ISecurityAttribute securityAttribute)
			{
				...
				foreach (var attribute in securityAttribute.Attributes) {
					...
					if (permission == null)
						continue;
					//not a trusted permission
					result.AddPermission(permission);
				}
				return result;
			}

			public System.Security.IPermission GetPermission(			/*?*/SecurityAction securityAction, ICustomAttribute attribute)
			{
			...
			public override void Visit(IPropertyDefinition propertyDefinition)
			{
				...
				foreach (var accessor in propertyDefinition.Accessors) {
					...
					if (accessor == propertyDefinition.Getter)
						propertyBuilder.SetGetMethod(accessorMethod); else if (accessor == propertyDefinition.Setter)
			...
			public void Visit(ITypeDefinition typeDefinition, TypeBuilder typeBuilder)
			{
				if (typeDefinition.HasDeclarativeSecurity) {
				...
				foreach (var explicitOverride in typeDefinition.ExplicitImplementationOverrides) {
					typeBuilder.DefineMethodOverride((MethodInfo)this.loader.mapper.GetMethod(explicitOverride.ImplementingMethod), (MethodInfo)this.loader.mapper.GetMethod(explicitOverride.ImplementedMethod));
				}
				if (typeDefinition.IsGeneric) {
					foreach (var genericParameter in typeDefinition.GenericParameters)
			...
			public void EmitIL(System.Reflection.Emit.ILGenerator ilGenerator, IMethodBody methodBody)
			{
				...
				foreach (IOperation operation in methodBody.Operations) {
					...
					Label label;
					if (labelFor.TryGetValue((uint)operation.Offset, out label))
					...
					switch (operation.OperationCode) {
						...
						case OperationCode.Calli:
							...
							foreach (var parameter in functionPointer.Parameters)
								parameterTypes[i++] = loader.mapper.GetType(parameter.Type);
							var optionalParameterTypes = new Type[IteratorHelper.EnumerableCount(functionPointer.ExtraArgumentTypes)];
							i = 0;
							foreach (var parameter in functionPointer.ExtraArgumentTypes)
								parameterTypes[i++] = loader.mapper.GetType(parameter.Type);
						...
						case OperationCode.Starg:
							if (operation.Value == null)
								//it's the this arg, which does not have an IParameterDefinition
			...
			public void EmitNamespaceScopes()
			{
			...
			public void CallAppropriateBeginsAndEnds(uint offset)
			{
				if (this.exceptionOffsets.Contains(offset)) {
					foreach (var exceptionInformation in this.methodBody.OperationExceptionInformation) {
						if (exceptionInformation.TryStartOffset == offset) {
							...
							//TODO: do we need to do anything with the label?
							...
							switch (exceptionInformation.HandlerKind) {
								case HandlerKind.Catch:
									this.ilGenerator.BeginCatchBlock(this.loader.mapper.GetType(exceptionInformation.ExceptionType));
									break;
								case HandlerKind.Fault:
									this.ilGenerator.BeginFaultBlock();
									break;
								case HandlerKind.Filter:
									this.ilGenerator.BeginCatchBlock(null);
									break;
				...
				if (this.scopeOffsets != null && this.scopeOffsets.Contains(offset)) {
					foreach (ILocalScope scope in this.localScopeProvider.GetLocalScopes(this.methodBody)) {
						if (scope.Offset == offset)
							this.ilGenerator.BeginScope(); else if (scope.Offset + scope.Length == offset)
							this.ilGenerator.EndScope();
					}
				}
			}

			public void CreateLabelsForBranchTargets()
			{
			...
			public void CreateLocalBuilders()
			{
				...
				foreach (var localDefinition in this.methodBody.LocalVariables) {
					...
					var localBuilder = this.ilGenerator.DeclareLocal(type, localDefinition.IsPinned);
					//there seems to be no way to emit modifiers
					if (this.sourceLocationProvider != null) {
						bool isCompilerGenerated;
						//no way to communicate this to Reflection.Emit, it seems.
			...
			public ISymbolDocumentWriter GetDocument(IDocument document)
			{
				if (this.currentDocument == document)
					return this.currentDocumentWriter;
				this.currentDocument = document;
				if (this.documentMap.TryGetValue(document, out this.currentDocumentWriter))
					return this.currentDocumentWriter;
			...
			public OpCode OpCodeFor(OperationCode operationCode)
			{
		...
		/// cyclic dependencies.
		/// </summary>
		public class TypeCreator : MetadataTraverser
		{

			public TypeCreator(DynamicLoader loader)
			{
				this.loader = loader;
			}
			...
			public void CreateTypesThatNeedToBeLoadedBeforeLoading(ITypeDefinition typeDefinition)
			{
				if (typeDefinition.IsGeneric) {
			...
			{
				object builder;
				if (!this.loader.builderMap.TryGetValue(namespaceTypeDefinition, out builder))
					return;
			...
			{
				object builder;
				if (!this.loader.builderMap.TryGetValue(nestedTypeDefinition, out builder))
					return;
