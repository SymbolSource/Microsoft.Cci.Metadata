...
namespace Microsoft.Cci.ReflectionEmitter
{
	...
	public class ReflectionMapper
	{
		...
		DoubleHashtable<FieldInfo> fieldMap = new DoubleHashtable<FieldInfo>(2048 * 4);
		//TODO: use a hashtable and the newly defined InternKey
		...
		public MethodBase GetMethod(		/*?*/			/*?*/IMethodReference methodReference		)
		{
			...
			if (genericMethodInstanceReference != null)
				return this.GetGenericMethodInstance(genericMethodInstanceReference);
		...
		public MethodBase GetMethodFrom(		/*?*/IMethodReference methodReference, MemberInfo[] members)
		{
			...
			foreach (var member in members) {
				...
				if (methodBase.IsConstructor) {
					...
					if (methodReturnType != method.ReturnType)
						continue;
				...
				for (int i = 0; i < parameterCount; i++) {
					...
					if (mparInfo.ParameterType != part) {
						matched = false;
						break;
					}
					if (ipar.IsModified) {
		...
		public bool CallingConventionsMatch(MethodBase methodBase, IMethodReference methodReference)
		{
			if (methodBase.IsStatic && !methodReference.IsStatic)
				return false;
			switch (methodBase.CallingConvention & (CallingConventions)3) {
				case CallingConventions.Any:
					if ((methodReference.CallingConvention & (CallingConvention)7) != CallingConvention.Default && (methodReference.CallingConvention & (CallingConvention)7) != CallingConvention.ExtraArguments)
						return false;
					break;
				...
				case CallingConventions.VarArgs:
					if ((methodReference.CallingConvention & (CallingConvention)7) != CallingConvention.ExtraArguments)
						return false;
					break;
			}
			if ((methodBase.CallingConvention & CallingConventions.HasThis) != 0 && (methodReference.CallingConvention & CallingConvention.HasThis) == 0)
				return false;
			if ((methodBase.CallingConvention & CallingConventions.ExplicitThis) != 0 && (methodReference.CallingConvention & CallingConvention.ExplicitThis) == 0)
				return false;
			return true;
		}

		public MethodInfo GetGenericMethodFrom(IMethodReference methodReference, MemberInfo[] members)
		{
			MethodInfo result = null;
			var parameterCount = methodReference.ParameterCount;
			var parameters = new IParameterTypeInformation[parameterCount];
			int i = 0;
			foreach (var par in methodReference.Parameters)
				parameters[i++] = par;
			...
			/*?*/		public bool TypesMatch(ITypeReference typeReference, Type type		, MethodInfo genericMethod)
		{
			if (type == null)
				return false;
			var arrayType = typeReference as IArrayTypeReference;
			if (arrayType != null)
				return this.TypesMatch(arrayType.ElementType, type.GetElementType(), genericMethod);
			var managedPointerType = typeReference as IManagedPointerTypeReference;
			if (managedPointerType != null)
				return this.TypesMatch(managedPointerType.TargetType, type.GetElementType(), genericMethod);
			var pointerType = typeReference as IPointerTypeReference;
			if (pointerType != null)
				return this.TypesMatch(pointerType.TargetType, type.GetElementType(), genericMethod);
			var modifiedType = typeReference as IModifiedTypeReference;
			if (modifiedType != null)
				return this.TypesMatch(modifiedType.UnmodifiedType, type, genericMethod);
			...
			if (genericMethodParameterReference != null) {
				...
				if (genericMethodParameterReference.Index >= genericMethodParameters.Length)
					return false;
			...
			if (genericTypeInstance != null) {
				if (!type.IsGenericType)
					return false;
				if (!this.TypesMatch(genericTypeInstance.GenericType, type.GetGenericTypeDefinition(), genericMethod))
					return false;
				var genericArguments = type.GetGenericArguments();
				if (genericArguments == null || genericArguments.Length != genericTypeInstance.GenericType.GenericParameterCount)
					return false;
				int i = 0;
				foreach (var genarg in genericTypeInstance.GenericArguments) {
					if (!this.TypesMatch(genarg, genericArguments[i++], genericMethod))
						return false;
				}
				return true;
			}
		...
		public MethodBase GetGenericMethodInstance(IGenericMethodInstanceReference genericMethodInstanceReference)
		{
		...
		public bool ModifiersMatch(Type[] optional, Type[] required, IEnumerable<ICustomModifier> modifiers)
		{
			int optionalCount = 0;
			int requiredCount = 0;
			int modifierCount = 0;
			foreach (var modifier in modifiers) {
				...
				if (modifier.IsOptional) {
					if (optionalCount == optional.Length)
						return false;
					if (optional[optionalCount++] != modifierType)
						return false;
				} else {
					if (requiredCount == required.Length)
						return false;
					if (required[requiredCount++] != modifierType)
						return false;
				}
				modifierCount++;
			}
		...
		public void DefineMapping(ITypeReference typeReference, Type type)
		{
			this.typeMap.Add(typeReference.InternedKey, type);
		}

		public void DefineMapping(IFieldDefinition fieldDefinition, FieldInfo fieldBuilder)
		{
			this.fieldMap.Add(fieldDefinition.ContainingType.InternedKey, (uint)fieldDefinition.Name.UniqueKey, fieldBuilder);
		}

		public void DefineMapping(IMethodDefinition method, MethodBase methodBuilder)
		{
			this.methodMap.Add(method.InternedKey, methodBuilder);
		}

		public MethodInfo GetArrayAddrMethod(IArrayTypeReference arrayTypeReference, ModuleBuilder moduleBuilder)
		{
		...
		public MethodInfo GetArrayGetMethod(IArrayTypeReference arrayTypeReference, ModuleBuilder moduleBuilder)
		{
			var type = this.GetType(arrayTypeReference);
			var parameterTypes = new Type[arrayTypeReference.Rank];
			for (int i = 0; i < arrayTypeReference.Rank; i++)
		...
		public MethodInfo GetArraySetMethod(IArrayTypeReference arrayTypeReference, ModuleBuilder moduleBuilder)
		{
			var type = this.GetType(arrayTypeReference);
		...
		public MethodInfo GetArrayCreateMethod(IArrayTypeReference arrayTypeReference, ModuleBuilder moduleBuilder)
		{
			var type = this.GetType(arrayTypeReference);
			var parameterTypes = new Type[arrayTypeReference.Rank];
			for (int i = 0; i < arrayTypeReference.Rank; i++)
		...
		public MethodInfo GetArrayCreateWithLowerBoundsMethod(IArrayTypeReference arrayTypeReference, ModuleBuilder moduleBuilder)
		{
			var type = this.GetType(arrayTypeReference);
	...
	/// </summary>
	public class MappingVisitorForTypes : MetadataVisitor
	{

		public MappingVisitorForTypes(ReflectionMapper mapper)
		{
			this.mapper = mapper;
		}
		...
		public void GetConsolidatedTypeArguments(List<Type> consolidatedTypeArguments, ITypeReference typeReference)
		{
