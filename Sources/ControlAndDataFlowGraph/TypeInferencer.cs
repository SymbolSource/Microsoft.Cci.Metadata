...
namespace Microsoft.Cci.ControlAndDataFlowGraph
{
	...
	/// 
	/// </summary>
	public class TypeInferencer<BasicBlock, Instruction> where BasicBlock : Microsoft.Cci.BasicBlock<Instruction>, new() where Instruction : Microsoft.Cci.Instruction, new()
	{

		public TypeInferencer(IMetadataHost host, ControlAndDataFlowGraph<BasicBlock, Instruction> cfg, Stack<Instruction> stack, Queue<BasicBlock> blocksToVisit, SetOfObjects blocksAlreadyVisited)
		{
		...
		/// 
		/// </summary>
		public static void FillInTypes(IMetadataHost host, ControlAndDataFlowGraph<BasicBlock, Instruction> cfg)
		{
			Contract.Requires(host != null);
			Contract.Requires(cfg != null);
		...
		public void DequeueBlockAndFillInItsTypes()
		{
			...
			//this.blocksToVisit only has non null elements, but we can't put that in a contract that satisfies the checker
			if (!this.blocksAlreadyVisited.Add(block))
				return;
			//The same block can be added multiple times to the queue.
			...
			foreach (var instruction in block.Instructions) {
				...
				//block.Instructions only has non null elements, but we can't put that in a contract that satisfies the checker
			...
			foreach (var successor in this.cfg.SuccessorsFor(block)) {
				...
				//block.Successors only has non null elements, but we can't put that in a contract that satisfies the checker
				this.TransferTypesFromStackTo(successor);
				if (blocksAlreadyVisited.Contains(successor))
					continue;
				blocksToVisit.Enqueue(successor);
				//The block might already be in the queue, but we can deal with this more efficiently by checking blocksAlreadyVisited when dequeueing.
			}

			this.stack.Clear();
		}

		public void TransferTypesFromStackTo(BasicBlock successor)
		{
			Contract.Requires(successor != null);
			Contract.Assume(this.stack.Top + 1 == successor.OperandStack.Count);
			//We assume that the DataFlowInferencer sets things up this way.
		...
		public void InferTypeAndUpdateStack(Instruction instruction)
		{
			...
			switch (instruction.Operation.OperationCode) {
				...
				case OperationCode.Calli:
					...
					Contract.Assume(funcPointer != null);
					//This is an informally specified property of the Metadata model.
					this.stack.Pop();
				...
				case OperationCode.Ldfld:
					this.stack.Pop();
					Contract.Assume(instruction.Operation.Value is IFieldReference);
					//This is an informally specified property of the Metadata model.
				...
				case OperationCode.Ldftn:
					Contract.Assume(instruction.Operation.Value is IMethodReference);
					//This is an informally specified property of the Metadata model.
				...
				case OperationCode.Ldind_Ref:
					...
					if (ptr != null)
						...
						Contract.Assume(instruction.Operand1.Type is IManagedPointerTypeReference);
						//This is an informally specified property of the Metadata model.
				...
				case OperationCode.Ldtoken:
					if (instruction.Operation.Value is IMethodReference)
						instruction.Type = this.platformType.SystemRuntimeMethodHandle; else if (instruction.Operation.Value is ITypeReference)
						instruction.Type = this.platformType.SystemRuntimeTypeHandle; else if (instruction.Operation.Value is IFieldReference)
				...
				case OperationCode.Newobj:
					...
					Contract.Assume(constructorReference != null);
					//This is an informally specified property of the Metadata model.
		...
		public ITypeReference GetUnsignedBinaryNumericOperationType(Instruction instruction)
		{
			Contract.Requires(instruction != null);
			return TypeHelper.UnsignedEquivalent(this.GetBinaryNumericOperationType(instruction));
		}

		public ITypeReference GetBinaryNumericOperationType(Instruction instruction)
		{
			Contract.Requires(instruction != null);
			var leftOperand = instruction.Operand1;
			var rightOperand = (Instruction)instruction.Operand2;
			Contract.Assume(leftOperand != null);
			//Assumed because of the informal specification of the DataFlowInferencer
			...
			switch (leftTypeCode) {
				...
				case PrimitiveTypeCode.UInt8:
					switch (rightTypeCode) {
						...
						case PrimitiveTypeCode.Int32:
							return this.platformType.SystemUInt32;
						//code generators will tend to make both operands be of the same type. Assume this happened because the right operand is a polymorphic constant.
						...
						//code generators will tend to make both operands be of the same type. Assume this happened because the right operand is an enum.
					}
				...
				case PrimitiveTypeCode.Int32:
					switch (rightTypeCode) {
						...
						case PrimitiveTypeCode.UInt8:
							return this.platformType.SystemUInt32;
						//code generators will tend to make both operands be of the same type. Assume this happened because the left operand is a polymorphic constant.
						case PrimitiveTypeCode.Int8:
						case PrimitiveTypeCode.Int16:
						case PrimitiveTypeCode.Int32:
