...
namespace Microsoft.Cci
{
	...
	/// </summary>
	public class ControlAndDataFlowGraph<BasicBlock, Instruction> where BasicBlock : Microsoft.Cci.BasicBlock<Instruction>, new() where Instruction : Microsoft.Cci.Instruction, new()
	{

		public ControlAndDataFlowGraph(IMethodBody body, List<BasicBlock> successorEdges, List<BasicBlock> allBlocks, List<BasicBlock> rootBlocks, Hashtable<BasicBlock> blockFor)
		{
		...
		public Sublist<BasicBlock> SuccessorsFor(BasicBlock basicBlock)
		{
			...
			//can only happen if the basic block does not belong to this graph.
	...
	public class BasicBlock<Instruction>
	{
		...
		//TODO: should we have explicit Phi nodes?
		/// <summary>
	...
	public class Instruction
	{
			...
			if (i2 != null) {
				...
				if (i2a != null) {
					foreach (var i2e in i2a) {
						Contract.Assume(i2e != null);
						//Assumed because of the informal specification of the ControlFlowInferencer
						stringBuilder.Append(", ");
		...
		public void AppendFlowFrom(Instruction instruction, StringBuilder stringBuilder)
		{
...
namespace Microsoft.Cci.ControlAndDataFlowGraph
{
	public class Stack<Instruction> where Instruction : class
	{
		...
		public Stack(int maxStack, List<Instruction> operandStackSetup)
		{
			Contract.Requires(operandStackSetup != null);
			Contract.Assume(Contract.ForAll(operandStackSetup, x => x != null));
			//Making this an explicit requirement creates too many proof obligations.
			if (maxStack <= 0)
				maxStack = 8;
		...
		public void Push(Instruction instruction)
		{
			...
			if (this.top >= this.elements.Length - 1) {
				...
				Contract.Assume(Contract.ForAll(0, this.top + 1, i => this.elements[i] != null));
				//this the expected behavior of Array.Resize
			}
		...
		public Instruction Pop()
		{
			...
			Contract.Assume(this.top >= 0);
			//This is an optimistic assumption. Clients have to match their Pop and Push calls, but enforcing this convention via contracts is too verbose.
