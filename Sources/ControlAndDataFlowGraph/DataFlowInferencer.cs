...
namespace Microsoft.Cci.ControlAndDataFlowGraph
{
	...
	public class DataFlowInferencer<BasicBlock, Instruction> where BasicBlock : Microsoft.Cci.BasicBlock<Instruction>, new() where Instruction : Microsoft.Cci.Instruction, new()
	{
		...
		public DataFlowInferencer(IMetadataHost host, ControlAndDataFlowGraph<BasicBlock, Instruction> cdfg)
		{
		...
		/// 
		/// </summary>
		public static void SetupDataFlow(IMetadataHost host, IMethodBody methodBody, ControlAndDataFlowGraph<BasicBlock, Instruction> cdfg)
		{
			Contract.Requires(host != null);
		...
		public void SetupDataFlowFor(IMethodBody methodBody)
		{
			Contract.Requires(methodBody != null);
		...
		public void AddStackSetupForExceptionHandlers(IMethodBody methodBody)
		{
			Contract.Requires(methodBody != null);
		...
		public void AddStackSetup(BasicBlock block, ITypeReference operandType)
		{
		...
		public void DequeueBlockAndSetupDataFlow()
		{
		...
		public void SetupStackFor(BasicBlock successor)
		{
			...
			if (successor.OperandStack.Count == 0) {
				...
				int n = this.stack.Top;
				Contract.Assume(n == successor.OperandStack.Count - 1);
				//This is an optimistic assumption. It should be true for any well formed PE file. We are content to crash given bad input.
				for (int i = 0; i <= n; i++) {
					var pushInstruction = this.stack.Peek(i);
					var setupInstruction = successor.OperandStack[i];
					Contract.Assume(setupInstruction.Operand2 is List<Instruction>);
					//This is set up in the successor.OperandStack.Count == 0, but is hard to write a contract to that effect.
		...
		public void SetupDataFlowFor(Instruction instruction)
		{
		...
		public static void InitializeArgumentsAndPushReturnResult(Instruction instruction, Stack<Instruction> stack, ISignature signature)
		{
			Contract.Requires(instruction != null);
		...
		public static void InitializeArgumentsAndPushReturnResult(Instruction instruction, Stack<Instruction> stack, IFunctionPointerTypeReference funcPointer)
		{
			Contract.Requires(instruction != null);
			Contract.Requires(stack != null);
		...
		public static void InitializeArrayCreateInstruction(Instruction instruction, Stack<Instruction> stack, IOperation currentOperation)
		{
			Contract.Requires(instruction != null);
			Contract.Requires(stack != null);
		...
		public static void InitializeArrayIndexerInstruction(Instruction instruction, Stack<Instruction> stack, IArrayTypeReference arrayType)
		{
			Contract.Requires(instruction != null);
			Contract.Requires(stack != null);
