...
namespace Microsoft.Cci.MetadataReader
{
	...
	public class DefaultWindowsRuntimeHost : WindowsRuntimeMetadataReaderHost
	{
		...
		public DefaultWindowsRuntimeHost(bool projectToCLRTypes = true) : base(new NameTable(), new InternFactory(), 0, null, true, projectToCLRTypes)
		{
		...
		/// <param name="projectToCLRTypes">True if the host should project references to certain Windows Runtime types and methods
		/// to corresponding CLR types and methods, in order to emulate the runtime behavior of the CLR.</param>
		public DefaultWindowsRuntimeHost(INameTable nameTable, bool projectToCLRTypes = true) : base(nameTable, new InternFactory(), 0, null, false, projectToCLRTypes)
		{
			this.peReader = new PeReader(this);
		}
	...
	public abstract class WindowsRuntimeMetadataReaderHost : MetadataReaderHost
	{
		...
		/// <param name="projectToCLRTypes">True if the host should project references to certain Windows Runtime types and methods
		/// to corresponding CLR types and methods, in order to emulate the runtime behavior of the CLR.</param>
		public WindowsRuntimeMetadataReaderHost(INameTable nameTable, IInternFactory factory, byte pointerSize, IEnumerable<string> searchPaths, bool searchInGAC, bool projectToCLRTypes) : base(nameTable, factory, pointerSize, searchPaths, searchInGAC)
		{
		...
		/// </summary>
		/// <returns></returns>
		public override IPlatformType GetPlatformType()
		{
		...
		public override IMethodDefinition Rewrite(IUnit containingUnit, IMethodDefinition methodDefinition)
		{
			...
			if (containingType == null || !containingType.IsForeignObject)
				return methodDefinition;
		...
		public override IEnumerable<IAssemblyReference> Redirect(IUnit referringUnit, IEnumerable<IAssemblyReference> assemblyReferences)
		{
			...
			foreach (var aref in assemblyReferences) {
				if (string.Equals(aref.Name.Value, "mscorlib", StringComparison.OrdinalIgnoreCase))
					continue;
				result.Add(aref);
				standardRefs.Remove(aref.AssemblyIdentity);
			}
			if (standardRefs.ContainsKey(platformType.CoreAssemblyRef.AssemblyIdentity))
				result.Add(platformType.CoreAssemblyRef);
			if (standardRefs.ContainsKey(platformType.SystemRuntimeInteropServicesWindowsRuntime.AssemblyIdentity))
				result.Add(platformType.SystemRuntimeInteropServicesWindowsRuntime);
			if (standardRefs.ContainsKey(platformType.SystemRuntimeWindowsRuntime.AssemblyIdentity))
				result.Add(platformType.SystemRuntimeWindowsRuntime);
			if (standardRefs.ContainsKey(platformType.SystemRuntimeWindowsRuntimeUIXaml.AssemblyIdentity))
				result.Add(platformType.SystemRuntimeWindowsRuntimeUIXaml);
			if (standardRefs.ContainsKey(platformType.SystemObjectModel.AssemblyIdentity))
				result.Add(platformType.SystemObjectModel);
		...
		public override ITypeReference Redirect(IUnit definingUnit, ITypeReference typeReference)
		{
			...
			if (definingAssembly == null || !(definingAssembly.ContainsForeignTypes))
				return typeReference;
			...
			if (this.IsWindowsFoundationMetadata(namespaceReference)) {
				if (namespaceTypeReference.Name == platformType.SystemAttributeUsageAttribute.Name)
					return platformType.SystemAttributeUsageAttribute;
				if (namespaceTypeReference.Name == platformType.SystemAttributeTargets.Name)
					return platformType.SystemAttributeTargets;
			} else if (this.IsWindowsUI(namespaceReference)) {
				if (namespaceTypeReference.Name == platformType.WindowsUIColor.Name)
					return platformType.WindowsUIColor;
			} else if (this.IsWindowsFoundation(namespaceReference)) {
				if (namespaceTypeReference.Name == platformType.SystemDateTime.Name)
					return platformType.SystemDateTimeOffset;
				...
				if (namespaceTypeReference.Name == this.HResult)
					return platformType.SystemException;
				...
				if (namespaceTypeReference.Name == platformType.WindowsFoundationPoint.Name)
					return platformType.WindowsFoundationPoint;
				if (namespaceTypeReference.Name == platformType.WindowsFoundationRect.Name)
					return platformType.WindowsFoundationRect;
				if (namespaceTypeReference.Name == platformType.WindowsFoundationSize.Name)
					return platformType.WindowsFoundationSize;
				if (namespaceTypeReference.Name == platformType.SystemTimeSpan.Name)
					return platformType.SystemTimeSpan;
				if (namespaceTypeReference.Name == platformType.SystemUri.Name)
					return platformType.SystemUri;
				if (namespaceTypeReference.Name == this.IClosable)
					return platformType.SystemIDisposable;
			} else if (this.IsWindowsFoundationCollections(namespaceReference)) {
				if (namespaceTypeReference.Name == this.IIterable)
					return platformType.SystemCollectionsGenericIEnumerable;
				if (namespaceTypeReference.Name == this.IVector)
					return platformType.SystemCollectionsGenericIList;
				if (namespaceTypeReference.Name == this.IVectorView)
					return platformType.SystemCollectionsGenericReadOnlyList;
				if (namespaceTypeReference.Name == this.IMap)
					return platformType.SystemCollectionsGenericIDictionary;
				if (namespaceTypeReference.Name == this.IMapView)
					return platformType.SystemCollectionsGenericReadOnlyDictionary;
				if (namespaceTypeReference.Name == this.IKeyValuePair)
					return platformType.SystemCollectionsGenericKeyValuePair;
			} else if (this.IsWindowsUIXamlInput(namespaceReference)) {
				if (namespaceTypeReference.Name == platformType.SystemWindowsInputICommand.Name)
					return platformType.SystemWindowsInputICommand;
			} else if (this.IsWindowsUIXamlInterop(namespaceReference)) {
				if (namespaceTypeReference.Name == this.IBindableIterable)
					return platformType.SystemCollectionsIEnumerable;
				if (namespaceTypeReference.Name == this.IBindableVector)
					return platformType.SystemCollectionsIList;
				if (namespaceTypeReference.Name == this.INotifyCollectionChanged)
					return platformType.SystemCollectionsSpecializedINotifyColletionChanged;
				if (namespaceTypeReference.Name == this.NotifyCollectionChangedEventHandler)
					return platformType.SystemCollectionsSpecializedNotifyCollectionChangedEventHandler;
				if (namespaceTypeReference.Name == this.NotifyCollectionChangedEventArgs)
					return platformType.SystemCollectionsSpecializedNotifyCollectionChangedEventArgs;
				if (namespaceTypeReference.Name == this.NotifyCollectionChangedAction)
					return platformType.SystemCollectionsSpecializedNotifyCollectionChangedAction;
				if (namespaceTypeReference.Name == this.TypeName)
					return platformType.SystemType;
			} else if (this.IsWindowsUIXamlData(namespaceReference)) {
				if (namespaceTypeReference.Name == this.INotifyPropertyChanged)
					return platformType.SystemComponentModelINotifyPropertyChanged;
				if (namespaceTypeReference.Name == this.PropertyChangedEventArgs)
					return platformType.SystemComponentModelPropertyChangedEventArgs;
				if (namespaceTypeReference.Name == this.PropertyChangedEventHandler)
					return platformType.SystemComponentModelPropertyChangedEventHandler;
			} else if (this.IsWindowsUIXaml(namespaceReference)) {
				if (namespaceTypeReference.Name == platformType.WindowsUIXamlCornerRadius.Name)
					return platformType.WindowsUIXamlCornerRadius;
				if (namespaceTypeReference.Name == platformType.WindowsUIXamlDuration.Name)
					return platformType.WindowsUIXamlDuration;
				if (namespaceTypeReference.Name == platformType.WindowsUIXamlDurationType.Name)
					return platformType.WindowsUIXamlDurationType;
				if (namespaceTypeReference.Name == platformType.WindowsUIXamlGridLength.Name)
					return platformType.WindowsUIXamlGridLength;
				if (namespaceTypeReference.Name == platformType.WindowsUIXamlGridUnitType.Name)
					return platformType.WindowsUIXamlGridUnitType;
				if (namespaceTypeReference.Name == platformType.WindowsUIXamlThickness.Name)
					return platformType.WindowsUIXamlThickness;
				...
				if (namespaceTypeReference.Name == platformType.WindowsUIXamlMediaMatrix.Name)
					return platformType.WindowsUIXamlMediaMatrix;
		...
		public override ICustomAttribute Rewrite(IUnit referringUnit, ICustomAttribute customAttribute)
		{
			...
			if (referringAssembly == null || !(referringAssembly.ContainsForeignTypes))
				return customAttribute;
			if (!TypeHelper.TypesAreEquivalent(customAttribute.Type, this.PlatformType.SystemAttributeUsageAttribute))
				return customAttribute;
			...
			if (argConst == null || !(argConst.value is int))
				return customAttribute;
			int clrEnumValue = 0;
		...
		public bool FellowCustomAttributeIncludeAllowMultiple(CustomAttribute customAttr)
		{
		...
		public override AssemblyIdentity UnifyAssembly(AssemblyIdentity assemblyIdentity)
		{
			...
			/*?*/		public bool IsWindows(INestedUnitNamespaceReference namespaceReference		)
		{
			if (namespaceReference == null || namespaceReference.Name != this.Windows)
				return false;
			return namespaceReference.ContainingUnitNamespace is IRootUnitNamespaceReference;
		}

			/*?*/		public bool IsWindowsFoundation(INestedUnitNamespaceReference namespaceReference		)
		{
			if (namespaceReference == null || namespaceReference.Name != this.Foundation)
				return false;
			return IsWindows(namespaceReference.ContainingUnitNamespace as INestedUnitNamespaceReference);
		}

			/*?*/		public bool IsWindowsFoundationCollections(INestedUnitNamespaceReference namespaceReference		)
		{
			if (namespaceReference == null || namespaceReference.Name != this.Collections)
				return false;
			return IsWindowsFoundation(namespaceReference.ContainingUnitNamespace as INestedUnitNamespaceReference);
		}

			/*?*/		public bool IsWindowsFoundationMetadata(INestedUnitNamespaceReference namespaceReference		)
		{
			if (namespaceReference == null || namespaceReference.Name != this.Metadata)
				return false;
			return IsWindowsFoundation(namespaceReference.ContainingUnitNamespace as INestedUnitNamespaceReference);
		}

			/*?*/		public bool IsWindowsUI(INestedUnitNamespaceReference namespaceReference		)
		{
			if (namespaceReference == null || namespaceReference.Name != this.UI)
				return false;
			return IsWindows(namespaceReference.ContainingUnitNamespace as INestedUnitNamespaceReference);
		}

			/*?*/		public bool IsWindowsUIXaml(INestedUnitNamespaceReference namespaceReference		)
		{
			if (namespaceReference == null || namespaceReference.Name != this.Xaml)
				return false;
			return IsWindowsUI(namespaceReference.ContainingUnitNamespace as INestedUnitNamespaceReference);
		}

			/*?*/		public bool IsWindowsUIXamlControls(INestedUnitNamespaceReference namespaceReference		)
		{
			if (namespaceReference == null || namespaceReference.Name != this.Controls)
				return false;
			return IsWindowsUIXaml(namespaceReference.ContainingUnitNamespace as INestedUnitNamespaceReference);
		}

			/*?*/		public bool IsWindowsUIXamlControlsPrimitives(INestedUnitNamespaceReference namespaceReference		)
		{
			if (namespaceReference == null || namespaceReference.Name != this.Primitives)
				return false;
			return IsWindowsUIXamlControls(namespaceReference.ContainingUnitNamespace as INestedUnitNamespaceReference);
		}

			/*?*/		public bool IsWindowsUIXamlData(INestedUnitNamespaceReference namespaceReference		)
		{
			if (namespaceReference == null || namespaceReference.Name != this.Data)
				return false;
			return IsWindowsUIXaml(namespaceReference.ContainingUnitNamespace as INestedUnitNamespaceReference);
		}

			/*?*/		public bool IsWindowsUIXamlInput(INestedUnitNamespaceReference namespaceReference		)
		{
			if (namespaceReference == null || namespaceReference.Name != this.Input)
				return false;
			return IsWindowsUIXaml(namespaceReference.ContainingUnitNamespace as INestedUnitNamespaceReference);
		}

			/*?*/		public bool IsWindowsUIXamlInterop(INestedUnitNamespaceReference namespaceReference		)
		{
			if (namespaceReference == null || namespaceReference.Name != this.Interop)
				return false;
			return IsWindowsUIXaml(namespaceReference.ContainingUnitNamespace as INestedUnitNamespaceReference);
		}

			/*?*/		public bool IsWindowsUIXamlMedia(INestedUnitNamespaceReference namespaceReference		)
		{
			if (namespaceReference == null || namespaceReference.Name != this.Media)
				return false;
			return IsWindowsUIXaml(namespaceReference.ContainingUnitNamespace as INestedUnitNamespaceReference);
		}

			/*?*/		public bool IsWindowsUIXamlMediaAnimation(INestedUnitNamespaceReference namespaceReference		)
		{
			if (namespaceReference == null || namespaceReference.Name != this.Animation)
				return false;
			return IsWindowsUIXamlMedia(namespaceReference.ContainingUnitNamespace as INestedUnitNamespaceReference);
		}

			/*?*/		public bool IsWindowsUIXamlMediaMedia3D(INestedUnitNamespaceReference namespaceReference		)
		{
			if (namespaceReference == null || namespaceReference.Name != this.Media3D)
				return false;
			return IsWindowsUIXamlMedia(namespaceReference.ContainingUnitNamespace as INestedUnitNamespaceReference);
		}
	...
	public class WindowsRuntimePlatform : PlatformType
	{
		...
		/// </param>
		public WindowsRuntimePlatform(IMetadataHost host) : base(host)
		{
		}
		...
		/// </summary>
		public IAssemblyReference System {
			get {
				if (this.system == null)
		...
		/// </summary>
		public AssemblyIdentity GetSystemSymbolicIdentity()
		{
		...
		/// </summary>
		public IAssemblyReference SystemObjectModel {
			get {
		...
		/// Returns an identity that is the same as CoreAssemblyIdentity, except that the name is "System".
		/// </summary>
		public AssemblyIdentity GetSystemObjectModelSymbolicIdentity()
		{
			var core = this.host.CoreAssemblySymbolicIdentity;
			var name = this.host.NameTable.System;
			var location = "";
		...
		/// </summary>
		public IAssemblyReference SystemRuntimeInteropServicesWindowsRuntime {
			get {
		...
		/// </summary>
		public AssemblyIdentity GetSystemRuntimeInteropServicesWindowsRuntimeSymbolicIdentity()
		{
			var core = this.host.CoreAssemblySymbolicIdentity;
		...
		/// </summary>
		public IAssemblyReference SystemRuntimeWindowsRuntime {
			get {
		...
		/// </summary>
		public AssemblyIdentity GetSystemRuntimeWindowsRuntimeSymbolicIdentity()
		{
			var core = this.host.CoreAssemblySymbolicIdentity;
		...
		/// </summary>
		public IAssemblyReference SystemRuntimeWindowsRuntimeUIXaml {
			get {
		...
		/// </summary>
		public AssemblyIdentity GetSystemRuntimeWindowsRuntimeUIXamlSymbolicIdentity()
		{
			var core = this.host.CoreAssemblySymbolicIdentity;
		...
		/// </summary>
		public INamespaceTypeReference SystemAttributeTargets {
			get {
		...
		/// </summary>
		public INamespaceTypeReference SystemCollectionsGenericIDictionary {
			get {
		...
		/// </summary>
		public INamespaceTypeReference SystemCollectionsGenericKeyValuePair {
			get {
				if (this.systemCollectionsGenericKeyValuePair == null) {
					this.systemCollectionsGenericKeyValuePair = this.CreateReference(this.CoreAssemblyRef, true, 2, PrimitiveTypeCode.NotPrimitive, "System", "Collections", "Generic", "KeyValuePair");
				}
		...
		/// </summary>
		public INamespaceTypeReference SystemCollectionsGenericReadOnlyDictionary {
			get {
		...
		/// System.IDisposable
		/// </summary>
		public INamespaceTypeReference SystemIDisposable {
			get {
		...
		/// System.Nullable`1
		/// </summary>
		public INamespaceTypeReference SystemNullable1 {
			get {
		...
		/// </summary>
		public INamespaceTypeReference SystemRuntimeInteropServicesWindowsRuntimeEventRegistrationToken {
			get {
				if (this.systemRuntimeInteropServicesWindowsRuntimeEventRegistrationToken == null) {
					if (string.Equals(this.CoreAssemblyRef.Name.Value, "System.Runtime", StringComparison.OrdinalIgnoreCase))
						this.systemRuntimeInteropServicesWindowsRuntimeEventRegistrationToken = this.CreateReference(this.SystemRuntimeInteropServicesWindowsRuntime, true, "System", "Runtime", "InteropServices", "WindowsRuntime", "EventRegistrationToken");
					else
						this.systemRuntimeInteropServicesWindowsRuntimeEventRegistrationToken = this.CreateReference(this.CoreAssemblyRef, true, "System", "Runtime", "InteropServices", "WindowsRuntime", "EventRegistrationToken");
				}
		...
		/// System.TimeSpan
		/// </summary>
		public INamespaceTypeReference SystemTimeSpan {
			get {
		...
		/// System.Uri
		/// </summary>
		public INamespaceTypeReference SystemUri {
			get {
		...
		/// </summary>
		public INamespaceTypeReference SystemWindowsInputICommand {
			get {
		...
		public INamespaceTypeReference WindowsUIXamlControlsPrimitivesGeneratorPosition {
			get {
				if (this.windowsUIXamlControlsPrimitivesGeneratorPosition == null) {
					this.windowsUIXamlControlsPrimitivesGeneratorPosition = this.CreateReference(this.SystemRuntimeWindowsRuntimeUIXaml, true, "Windows", "UI", "Xaml", "Controls", "Primitives", "GeneratorPosition");
				}
		...
		public INamespaceTypeReference WindowsUIXamlMediaAnimationKeyTime {
			get {
				if (this.windowsUIXamlMediaAnimationKeyTime == null) {
					this.windowsUIXamlMediaAnimationKeyTime = this.CreateReference(this.SystemRuntimeWindowsRuntimeUIXaml, true, "Windows", "UI", "Xaml", "Media", "Animation", "KeyTime");
				}
		...
		public INamespaceTypeReference WindowsUIXamlMediaAnimationRepeatBehavior {
			get {
				if (this.windowsUIXamlMediaAnimationRepeatBehavior == null) {
					this.windowsUIXamlMediaAnimationRepeatBehavior = this.CreateReference(this.SystemRuntimeWindowsRuntimeUIXaml, true, "Windows", "UI", "Xaml", "Media", "Animation", "RepeatBehavior");
				}
		...
		public INamespaceTypeReference WindowsUIXamlMediaAnimationRepeatBehaviorType {
			get {
				if (this.windowsUIXamlMediaAnimationRepeatBehaviorType == null) {
					this.windowsUIXamlMediaAnimationRepeatBehaviorType = this.CreateReference(this.SystemRuntimeWindowsRuntimeUIXaml, true, "Windows", "UI", "Xaml", "Media", "Animation", "RepeatBehaviorType");
				}
		...
		public INamespaceTypeReference WindowsUIXamlMediaMedia3DMatrix3D {
			get {
				if (this.windowsUIXamlMediaMedia3DMatrix3D == null) {
					this.windowsUIXamlMediaMedia3DMatrix3D = this.CreateReference(this.SystemRuntimeWindowsRuntimeUIXaml, true, "Windows", "UI", "Xaml", "Media", "Media3D", "Matrix3D");
				}
	...
	public sealed partial class PEFileToObjectModel
	{
		...
		public IEnumerable<ICustomAttribute> GetAttributesForSameParentAs(uint tokenValue)
		{
			...
			while (i > 1 && table[i - 1].Parent == parent)
				i--;
			while (i < len) {
				if (i == index) {
					i++;
