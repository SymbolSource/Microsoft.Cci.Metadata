...
namespace Microsoft.Cci.Pdb
{
	public class PdbFile
	{
		...
		public static Dictionary<string, int> LoadNameIndex(BitAccess bits)
		{
		...
		public static void LoadFuncsFromDbiModule(BitAccess bits, DbiModuleInfo info, IntHashTable names, ArrayList funcList, bool readStrings, MsfDirectory dir, Dictionary<string, int> nameIndex, PdbReader reader)
		{
		...
		public static PdbFunction[] LoadFunctions(Stream read, out Dictionary<uint, PdbTokenLine> tokenToSourceMapping)
		{
			...
			if (header.snTokenRidMap != 0 && header.snTokenRidMap != 0xffff) {
				...
				foreach (PdbFunction func in funcs) {
					func.token = 0x6000000 | ridMap[func.token & 0xffffff];
				}
			}
