...
namespace Microsoft.Cci.Pdb
{
	public class BitAccess
	{
		...
		{
			value = (short)((buffer[offset + 0] & 0xff) | (buffer[offset + 1] << 8));
			offset += 2;
		}
		...
		{
			value = (int)((buffer[offset + 0] & 0xff) | (buffer[offset + 1] << 8) | (buffer[offset + 2] << 16) | (buffer[offset + 3] << 24));
			offset += 4;
		}

		public void ReadInt64(out long value)
		{
			value = (long)(((ulong)buffer[offset + 0] & 0xff) | ((ulong)buffer[offset + 1] << 8) | ((ulong)buffer[offset + 2] << 16) | ((ulong)buffer[offset + 3] << 24) | ((ulong)buffer[offset + 4] << 32) | ((ulong)buffer[offset + 5] << 40) | ((ulong)buffer[offset + 6] << 48) | ((ulong)buffer[offset + 7] << 56));
			offset += 8;
		}

		public void ReadUInt16(out ushort value)
		{
			value = (ushort)((buffer[offset + 0] & 0xff) | (buffer[offset + 1] << 8));
			offset += 2;
		}
		...
		{
			value = (uint)((buffer[offset + 0] & 0xff) | (buffer[offset + 1] << 8) | (buffer[offset + 2] << 16) | (buffer[offset + 3] << 24));
			offset += 4;
		}
		...
		public decimal ReadDecimal()
		{
			int[] bits = new int[4];
			this.ReadInt32(bits);
			return new decimal(bits[2], bits[3], bits[1], bits[0] < 0, (byte)((bits[0] & 0xff0000) >> 16));
		}
		...
		public void ReadGuid(out Guid guid)
		{
			...
			guid = new Guid(a, b, c, d, e, f, g, h, i, j,
			k);
		}
