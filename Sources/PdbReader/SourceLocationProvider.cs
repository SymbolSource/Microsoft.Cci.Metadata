...
namespace Microsoft.Cci
{
	...
	public sealed class PdbReader : ISourceLocationProvider, ILocalScopeProvider, IDisposable
	{
		...
		public PdbSlot GetSlotFor(		/*?*/PdbScope[] pdbScopes, uint index)
		{
			PdbSlot 			/*?*/result = null;
		...
		public PdbFunction GetPdbFunctionFor(		/*?*/IMethodBody methodBody)
		{
		...
		public static uint GetTokenFor(IMethodBody methodBody)
		{
		...
		public PdbFunction GetPdbFunctionFor(ILocalDefinition localDefinition)
		{
			PdbFunction 			/*?*/result = null;
			foreach (ILocation location in localDefinition.Locations) {
				IILLocation 				/*?*/mbLocation = location as IILLocation;
		...
		public void FillInScopesAndSubScopes(IMethodBody methodBody, PdbScope[] pdbScopes, List<ILocalScope> scopes)
		{
			foreach (PdbScope scope in pdbScopes) {
				...
				if (pdbFunction.usingCounts != null) {
					...
					if (this.pdbFunctionMap.TryGetValue(pdbFunction.tokenOfMethodWhoseUsingInfoAppliesToThisMethod, out func)) {
						if (func.usingCounts != null) {
							if (func.namespaceScopes != null)
								return func.namespaceScopes;
		...
		public List<INamespaceScope> GetNamespaceScopes(ushort[] usingCounts, PdbScope pdbScope)
		{
		...
		public IUsedNamespace GetUsedNamespace(string namespaceName)
		{
		...
		public ILocation LocationWithSmallerOffset(ILocation location1, ILocation location2)
		{
			...
			if (tmbl == null || tmbl.Offset > smbl.Offset)
				return location2;
			return location1;
		}

		public IPrimarySourceLocation MapMethodBodyLocationToSourceLocation(		/*?*/IILLocation mbLocation, bool exact)
		{
			...
			foreach (PdbLines pdbLines in pdbFunction.lines) {
				...
				while (minIndex <= maxIndex) {
					...
					if (midPointIndex == maxIndex || (mid.offset <= desiredOffset && desiredOffset < array[midPointIndex + 1].offset)) {
		...
		public PdbSourceDocument GetPrimarySourceDocumentFor(PdbSource pdbSourceFile)
		{
...
namespace Microsoft.Cci.Pdb
{
	...
	public sealed class LocalNameSourceLocation : IPrimarySourceLocation
	{

		public LocalNameSourceLocation(string source)
		{
			this.source = source;
		}
	...
	/// </summary>
	public sealed class PdbSourceDocument : PrimarySourceDocument
	{
		...
		public PdbSourceDocument(IName name, PdbSource pdbSourceFile, StreamReader streamReader) : base(name, pdbSourceFile.name, streamReader)
		{
		...
		/// <param name="name">The name of the document. Used to identify the document in user interaction.</param>
		/// <param name="pdbSourceFile">Information about the document, such as its location.</param>
		public PdbSourceDocument(IName name, PdbSource pdbSourceFile) : base(name, pdbSourceFile.name, "")
		{
			this.pdbSourceFile = pdbSourceFile;
		}
		...
		public override string SourceLanguage {
			get {
				if (this.Language == SymLanguageType.Basic)
					return "Basic";
				if (this.Language == SymLanguageType.C)
					return "C";
				if (this.Language == SymLanguageType.Cobol)
					return "Cobol";
				if (this.Language == SymLanguageType.CPlusPlus)
					return "C++";
				if (this.Language == SymLanguageType.CSharp)
					return "C#";
				if (this.Language == SymLanguageType.ILAssembly)
					return "ILAssembly";
				if (this.Language == SymLanguageType.Java)
					return "Java";
				if (this.Language == SymLanguageType.JScript)
					return "JScript";
				if (this.Language == SymLanguageType.MCPlusPlus)
					return "MC++";
				if (this.Language == SymLanguageType.Pascal)
					return "Pascal";
				if (this.Language == SymLanguageType.SMC)
					return "SMC";
				return "";
				//TODO: search registry based on file extension
			}
		}
	...
	/// </summary>
	public sealed class PdbSourceLineLocation : IPrimarySourceLocation
	{

		/// <summary>
		/// Allocates a range of source text that corresponds to a source line.
		/// </summary>
		public PdbSourceLineLocation(PdbSourceDocument primarySourceDocument, int startLine, int startColumn, int endLine, int endColumn)
		{
		...
		public int Length {
			get {
				...
				if (result < 0)
					result = this.EndColumn - this.StartColumn;
				return result;
			}
		}
		...
		public string Source {
			get { return this.primarySourceDocument.GetSourceLocation(this.StartIndex, this.Length).Source; }
		}
	...
	public sealed class PdbLocalConstant : ILocalDefinition
	{
		...
		public ITypeReference GetTypeForConstant()
		{
			...
			switch (ic.GetTypeCode()) {
				case TypeCode.Boolean:
					return platformType.SystemBoolean;
				case TypeCode.Byte:
					return platformType.SystemUInt8;
				case TypeCode.Char:
					return platformType.SystemChar;
				case TypeCode.Decimal:
					return platformType.SystemDecimal;
				case TypeCode.Double:
					return platformType.SystemFloat64;
				case TypeCode.Int16:
					return platformType.SystemInt16;
				case TypeCode.Int32:
					return platformType.SystemInt32;
				case TypeCode.Int64:
					return platformType.SystemInt64;
				case TypeCode.SByte:
					return platformType.SystemInt8;
				case TypeCode.Single:
					return platformType.SystemFloat64;
				case TypeCode.String:
					return platformType.SystemString;
				case TypeCode.UInt16:
					return platformType.SystemUInt16;
				case TypeCode.UInt32:
					return platformType.SystemUInt32;
				case TypeCode.UInt64:
					return platformType.SystemUInt64;
				default:
					return platformType.SystemObject;
			}
		}
		...
		//TODO: return a method body location or some such thing
	...
	public sealed class PdbMetadataConstant : IMetadataConstant
	{

		public PdbMetadataConstant(object value, ITypeReference type)
		{
			this.value = value;
			this.type = type;
		}
	...
	/// </summary>
	public sealed class PdbLocalScope : ILocalScope
	{

		/// <summary>
		/// Allocates a range of CLR IL operations that comprise a lexical scope, specified as an IL offset and a length.
		/// </summary>
		public PdbLocalScope(IMethodBody methodBody, PdbScope pdbScope)
		{
			this.methodBody = methodBody;
			this.pdbScope = pdbScope;
		}

		public readonly IMethodBody methodBody;

		public readonly PdbScope pdbScope;
