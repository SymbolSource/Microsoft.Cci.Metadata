    ...
    */
		...
		public void Insert(int key, Object nvalue, bool @add)
		{
			...
			do {
				...
				if (((buckets[bucketNumber].hash_coll & 0x7fffffff) == hashcode) && key == buckets[bucketNumber].key) {
					if (@add) {
				...
				if (emptySlotNumber == -1) {
					...
					if (buckets[bucketNumber].hash_coll >= 0) {
						buckets[bucketNumber].hash_coll |= unchecked((int)0x80000000u);
						occupancy++;
					}
				}
				seed += incr;
		...
		public void putEntry(bucket[] newBuckets, int key, Object nvalue, int hashcode)
		{
			...
			do {
				...
				if (newBuckets[bucketNumber].hash_coll >= 0) {
					newBuckets[bucketNumber].hash_coll |= unchecked((int)0x80000000u);
					occupancy++;
				}
				seed += incr;
			} while (true);
