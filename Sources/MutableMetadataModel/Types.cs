...
namespace Microsoft.Cci.MutableCodeModel
{
	...
	public abstract class ArrayTypeReference : TypeReference, IArrayTypeReference
	{
		...
		/// </summary>
		public abstract IArrayType Resolve();
	...
	public sealed class FunctionPointerTypeReference : TypeReference, IFunctionPointerTypeReference, ICopyFrom<IFunctionPointerTypeReference>
	{
		...
		public IFunctionPointer ResolvedFunctionPointer {
			get {
				...
				if (this.resolvedFunctionPointer == null) {
					...
					this.resolvedFunctionPointer = new Immutable.FunctionPointerType(this.callingConvention, this.returnValueIsByRef, this.type, self.ReturnValueCustomModifiers, self.Parameters, self.ExtraArgumentTypes, this.InternFactory);
		...
		IEnumerable<IParameterTypeInformation> IFunctionPointerTypeReference.ExtraArgumentTypes {
			...
			get {
				if (this.extraArgumentTypes == null)
					return Dummy.FunctionPointer.ExtraArgumentTypes;
		...
		IEnumerable<IParameterTypeInformation> ISignature.Parameters {
			...
			get {
				if (this.parameters == null)
					return Dummy.FunctionPointer.Parameters;
		...
		IEnumerable<ICustomModifier> ISignature.ReturnValueCustomModifiers {
			...
			get {
				if (this.returnValueCustomModifiers == null)
					return Dummy.FunctionPointer.ReturnValueCustomModifiers;
	...
	public sealed class GenericMethodParameterReference : TypeReference, IGenericMethodParameterReference, ICopyFrom<IGenericMethodParameterReference>
	{
		...
		public void Resolve()
		{
			...
			foreach (var par in definingMethod.GenericParameters) {
				if (par.Index == this.index) {
					this.resolvedType = par;
					return;
				}
			}
		}
	...
	public abstract class GenericParameter : NamedTypeDefinition, IGenericParameter, ICopyFrom<IGenericParameter>
	{
		...
		public ITypeDefinition GetEffectiveBaseClass()
		{
		...
		public override bool IsReferenceType {
			get {
				if (((int)this.flags & 0x800) == 0) {
					this.flags |= (NamedTypeDefinition.Flags)0x800;
	...
	public sealed class ManagedPointerTypeReference : TypeReference, IManagedPointerTypeReference, ICopyFrom<IManagedPointerTypeReference>
	{
		...
		public IManagedPointerType ResolvedManagedPointerType {
			get {
	...
	public sealed class MatrixTypeReference : ArrayTypeReference, ICopyFrom<IArrayTypeReference>
	{
		...
		/// </summary>
		/// <returns></returns>
		public override IArrayType Resolve()
		{
	...
	public sealed class NamespaceTypeDefinition : NamedTypeDefinition, INamespaceTypeDefinition, ICopyFrom<INamespaceTypeDefinition>
	{
		...
		/// <value></value>
		public bool IsPublic {
			get { return (((TypeMemberVisibility)this.flags) & TypeMemberVisibility.Mask) == TypeMemberVisibility.Public; }
			set {
		...
		IEnumerable<ICustomAttribute> INamespaceTypeDefinition.AttributesFor(ITypeReference implementedInterface)
		{
			...
			if (this.AttributesFor != null && this.AttributesFor.TryGetValue(implementedInterface, out result))
				return result;
	...
	public sealed class NamespaceTypeReference : TypeReference, INamespaceTypeReference, ICopyFrom<INamespaceTypeReference>
	{
		...
		public override IAliasForType AliasForType {
			get {
				if (this.aliasForType == null)
					this.resolvedType = this.Resolve();
				//Also initializes this.aliasForType
		...
		/// </summary>
		public INamespaceTypeDefinition Resolve()
		{
			Contract.Ensures(this.IsFrozen);
			this.isFrozen = true;
			...
			foreach (INamespaceMember member in this.ContainingUnitNamespace.ResolvedUnitNamespace.GetMembersNamed(this.name, false)) {
				...
				if (nsTypeDef != null) {
					if (nsTypeDef.GenericParameterCount == this.GenericParameterCount)
						return nsTypeDef;
				} else {
					var nsAlias = member as INamespaceAliasForType;
					if (nsAlias != null && nsAlias.AliasedType.GenericParameterCount == this.GenericParameterCount)
	...
	public class NestedTypeReference : TypeReference, INestedTypeReference, ICopyFrom<INestedTypeReference>
	{
		...
		public INestedTypeDefinition Resolve()
		{
					...
					if (this.ContainingType.IsAlias) {
						...
						if (assembly != null) {
							foreach (var alias in assembly.ExportedTypes) {
								...
								if (neAlias == null)
									continue;
								if (neAlias.Name.UniqueKey != this.Name.UniqueKey)
									continue;
								if (neAlias.ContainingAlias != this.ContainingType.AliasForType)
									continue;
	...
	public sealed class PointerTypeReference : TypeReference, IPointerTypeReference, ICopyFrom<IPointerTypeReference>
	{
		...
		/// </summary>
		/// <value>The type of the resolved pointer.</value>
		public IPointerType ResolvedPointerType {
			get {
	...
	public abstract class NamedTypeDefinition : INamedTypeDefinition, ICopyFrom<INamedTypeDefinition>
	{
		...
		public enum Flags
		{
			...
			Delegate = 0x10000000,
			Enum = 0x8000000,
			HasDeclarativeSecurity = 0x4000000,
			Interface = 0x2000000,
			Sealed = 0x1000000,
			Static = 0x800000,
			Struct = 0x400000,
			ValueType = 0x200000,
			IsRuntimeSpecialName = 0x100000,
			IsSpecialName = 0x80000,
			IsComObject = 0x40000,
			IsSerializable = 0x20000,
			IsBeforeFieldInit = 0x10000,
			MustBeReferenceType = 0x8000,
			MustBeValueType = 0x4000,
			MustHaveDefaultConstructor = 0x2000,
			MangleName = 0x1000,
			IsForeignObject = 0x800,
			None = 0x0
		}
		public Flags flags;
		...
		/// </summary>
		/// <returns></returns>
		public INamedTypeReference GetSpecializedType(INamedTypeDefinition typeDef)
		{
			...
			if (nestedType != null) {
				...
				if (nestedType.ContainingTypeDefinition.IsGeneric)
					...
					if (containingTypeReference == nestedType.ContainingTypeDefinition)
						return typeDef;
				}
		...
		IEnumerable<IEventDefinition> ITypeDefinition.Events {
			get {
				if (this.Events == null)
					return Enumerable<IEventDefinition>.Empty;
		...
		IEnumerable<IMethodImplementation> ITypeDefinition.ExplicitImplementationOverrides {
			get {
				if (this.ExplicitImplementationOverrides == null)
					return Enumerable<IMethodImplementation>.Empty;
		...
		IEnumerable<IFieldDefinition> ITypeDefinition.Fields {
			get {
				if (this.Fields == null)
					return Enumerable<IFieldDefinition>.Empty;
		...
		IEnumerable<INestedTypeDefinition> ITypeDefinition.NestedTypes {
			get {
				if (this.NestedTypes == null)
					return Enumerable<INestedTypeDefinition>.Empty;
		...
		public static IEnumerable<ITypeDefinitionMember> EnumerateTolerantly(List<ITypeDefinitionMember> list)
		{
		...
		IEnumerable<IPropertyDefinition> ITypeDefinition.Properties {
			get {
				if (this.Properties == null)
					return Enumerable<IPropertyDefinition>.Empty;
		...
		IEnumerable<ISecurityAttribute> ITypeDefinition.SecurityAttributes {
			get {
				if (this.SecurityAttributes == null)
					return Enumerable<ISecurityAttribute>.Empty;
	...
	public abstract class TypeReference : ITypeReference, ICopyFrom<ITypeReference>
	{
		...
		public void Copy(ITypeReference typeReference, IInternFactory internFactory)
		{
			...
			//the attributes of a type definition are not the same as the attributes of a type reference
			//so when a definition is being copied as a reference, it should get not attributes of its own.
				this.attributes = null; else if (IteratorHelper.EnumerableIsNotEmpty(typeReference.Attributes))
		...
		public abstract partial class TypeReferenceAbstractMethodContracts : TypeReference
		{
			public override void DispatchAsReference(IMetadataVisitor visitor)
			{
		...
		public bool IsValueType {
			get {
				if (this.originalReference != null)
					return this.originalReference.IsValueType;
		...
		public partial class TypeReferenceAbstractMethodContracts : TypeReference
		{
			public override ITypeDefinition ResolvedType {
				get {
					Contract.Ensures(Contract.Result<ITypeDefinition>() != null);
		...
		IEnumerable<ICustomAttribute> IReference.Attributes {
			get {
				if (this.Attributes == null)
					return Dummy.TypeReference.Attributes;
				return this.Attributes.AsReadOnly();
			}
		}
		...
		IEnumerable<ILocation> IObjectWithLocations.Locations {
			get {
				if (this.Locations == null)
					return Dummy.TypeReference.Locations;
				return this.Locations.AsReadOnly();
			}
		}
