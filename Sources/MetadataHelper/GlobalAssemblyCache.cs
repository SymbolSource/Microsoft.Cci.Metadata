...
namespace Microsoft.Cci
{
	...
	public static class GlobalAssemblyCache
	{
		...
		#if COMPACTX
		public static bool IdentityMatchesString(AssemblyIdentity assemblyIdentity, string gacName)
		{
			int n = gacName.Length;
			var i = 0;
			if (!MatchIgnoringCase(assemblyIdentity.Name.Value, ',', gacName, ref i, n))
				return false;
			while (i < n) {
				char ch = gacName[i];
				switch (ch) {
					case 'v':
					case 'V':
						if (!MatchIgnoringCase("Version", '=', gacName, ref i, n))
							return false;
						if (!MatchDecimal(assemblyIdentity.Version.Major, '.', gacName, ref i, n))
							return false;
						if (!MatchDecimal(assemblyIdentity.Version.MajorRevision, '.', gacName, ref i, n))
							return false;
						if (!MatchDecimal(assemblyIdentity.Version.Minor, '.', gacName, ref i, n))
							return false;
						if (!MatchDecimal(assemblyIdentity.Version.MinorRevision, ',', gacName, ref i, n))
							return false;
						break;
					case 'C':
					case 'c':
						if (!MatchIgnoringCase("Culture", '=', gacName, ref i, n))
							return false;
					...
					case 'P':
					case 'p':
						if (!MatchIgnoringCase("PublicKeyToken", '=', gacName, ref i, n))
							return false;
						if (!MatchHex(assemblyIdentity.PublicKeyToken, ',', gacName, ref i, n))
							return false;
						break;
					default:
						return false;
		...
		public static bool MatchHex(IEnumerable<byte> bytes, char delimiter, string gacName, ref int i, int n)
		{
			...
			if (i >= n)
				return true;
			if (gacName[i++] != delimiter)
				return false;
			SkipBlanks(gacName, ref i, n);
			return true;
		}

		public static void SkipBlanks(string gacName, ref int i, int n)
		{
			while (i < n && gacName[i] == ' ')
				i++;
		}

		public static bool MatchDecimal(int val, char delimiter, string gacName, ref int i, int n)
		{
			int num = 0;
		...
		public static bool MatchIgnoringCase(string str, char delimiter, string gacName, ref int i, int n)
		{
			var m = str.Length;
			int j = 0;
			while (j < m && i < n) {
				if (Char.ToLowerInvariant(str[j++]) != Char.ToLowerInvariant(gacName[i++]))
					return false;
			}
			if (j != m)
				return false;
	...
	public class AssemblyName
	{
		...
		public AssemblyName(IAssemblyName assemblyName)
		{
		...
		public string ReadString(uint assemblyNameProperty)
		{
			uint size = 0;
		...
		public ushort ReadUInt16(uint assemblyNameProperty)
		{
			uint size = 0;
			this.assemblyName.GetProperty(assemblyNameProperty, IntPtr.Zero, ref size);
			IntPtr ptr = Marshal.AllocHGlobal((int)size);
		...
		public byte[] ReadBytes(uint assemblyNameProperty)
		{
			uint size = 0;
			this.assemblyName.GetProperty(assemblyNameProperty, IntPtr.Zero, ref size);
			IntPtr ptr = Marshal.AllocHGlobal((int)size);
		...
		public class ASSEMBLYINFO_FLAG
		{
			public ASSEMBLYINFO_FLAG()
			{
			}
		...
		{
			[PreserveSig()]
			int UninstallAssembly(uint dwFlags, 			[MarshalAs(UnmanagedType.LPWStr)]
string pszAssemblyName, IntPtr pvReserved, int pulDisposition);
			[PreserveSig()]
			int QueryAssemblyInfo(uint dwFlags, 			[MarshalAs(UnmanagedType.LPWStr)]
string pszAssemblyName, ref ASSEMBLY_INFO pAsmInfo);
			[PreserveSig()]
			int CreateAssemblyCacheItem(uint dwFlags, IntPtr pvReserved, out object ppAsmItem, 			[MarshalAs(UnmanagedType.LPWStr)]
...
string pszManifestFilePath, IntPtr pvReserved);
	...
	public interface IAssemblyName
	{
		...
		[PreserveSig()]
		int BindToObject(object refIID, object pAsmBindSink, IApplicationContext pApplicationContext, 		[MarshalAs(UnmanagedType.LPWStr)]
string szCodeBase, long llFlags, int pvReserved, uint cbReserved, out int ppv);
		[PreserveSig()]
	...
	public interface IApplicationContext
	{
		...
		void Set(		[MarshalAs(UnmanagedType.LPWStr)]
string szName, int pvValue, uint cbValue, uint dwFlags);
		void Get(		[MarshalAs(UnmanagedType.LPWStr)]
string szName, out int pvValue, ref uint pcbValue, uint dwFlags);
