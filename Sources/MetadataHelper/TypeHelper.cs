...
namespace Microsoft.Cci
{
	...
	public static class ClrHelper
	{
		...
		public static ClrOperandStackType ClrOperandStackTypeFor(ITypeReference typeReference)
		{
			...
			if (typeDefinition.IsEnum)
				//should only be true for types that can be resolved
		...
		public static PrimitiveTypeCode PrimitiveTypeCodeFor(ClrOperandStackType numericType)
		{
			switch (numericType) {
				...
				case ClrOperandStackType.Pointer:
					return PrimitiveTypeCode.Pointer;
				default:
		...
		public static readonly ClrOperandStackType[,] AddResult = new ClrOperandStackType[,] {
			...
			{
				ClrOperandStackType.Int32,
				ClrOperandStackType.Invalid,
				ClrOperandStackType.NativeInt,
				ClrOperandStackType.Invalid,
				ClrOperandStackType.Reference,
				ClrOperandStackType.Invalid,
				ClrOperandStackType.Pointer,
				ClrOperandStackType.Invalid
			},
			{
				ClrOperandStackType.Invalid,
				ClrOperandStackType.Int64,
				ClrOperandStackType.Invalid,
				ClrOperandStackType.Invalid,
				ClrOperandStackType.Invalid,
				ClrOperandStackType.Invalid,
	...
	public static class TypeHelper
	{
		...
		public static ITypeDefinition BaseClass(		/*?*/		//^ ensures result == null || result.IsClass;
ITypeDefinition typeDef)
		...
		public static bool CanAccess(ITypeDefinition typeDefinition, ITypeDefinitionMember member)
		{
			...
			if (TypeHelper.TypesAreEquivalent(typeDefinition, member.ContainingTypeDefinition))
				return true;
			...
			if (geninst != null && TypeHelper.TypesAreEquivalent(typeDefinition, geninst.GenericType.ResolvedType))
				return true;
			if (!CanAccess(typeDefinition, member.ContainingTypeDefinition))
				return false;
			switch (member.Visibility) {
				...
				case TypeMemberVisibility.FamilyAndAssembly:
					return TypeHelper.GetDefiningUnit(typeDefinition).UnitIdentity.Equals(TypeHelper.GetDefiningUnit(member.ContainingTypeDefinition).UnitIdentity) && TypeHelper.Type1DerivesFromOrIsTheSameAsType2(typeDefinition, member.ContainingTypeDefinition);
				//TODO: friend assemblies
				case TypeMemberVisibility.FamilyOrAssembly:
					return TypeHelper.GetDefiningUnit(typeDefinition).UnitIdentity.Equals(TypeHelper.GetDefiningUnit(member.ContainingTypeDefinition)) || TypeHelper.Type1DerivesFromOrIsTheSameAsType2(typeDefinition, member.ContainingTypeDefinition);
				//TODO: friend assemblies
		...
		public static bool CanAccess(ITypeDefinition typeDefinition, ITypeDefinition otherTypeDefinition)
		{
			...
			if (TypeHelper.TypesAreEquivalent(typeDefinition, otherTypeDefinition))
				return true;
			...
			if (nsTypeDef != null) {
				if (nsTypeDef.IsPublic)
					return true;
				return TypeHelper.GetDefiningUnit(nsTypeDef).Equals(TypeHelper.GetDefiningUnit(typeDefinition));
				//TODO: worry about /addmodule
			}
			INestedTypeDefinition 			/*?*/nestedTypeDef = otherTypeDefinition as INestedTypeDefinition;
			if (nestedTypeDef != null)
				return CanAccess(typeDefinition, (ITypeDefinitionMember)nestedTypeDef);
			IManagedPointerType 			/*?*/managedPointerType = otherTypeDefinition as IManagedPointerType;
			if (managedPointerType != null)
				return CanAccess(typeDefinition, managedPointerType.TargetType.ResolvedType);
			IPointerType 			/*?*/pointerType = otherTypeDefinition as IPointerType;
			if (pointerType != null)
				return CanAccess(typeDefinition, pointerType.TargetType.ResolvedType);
			IArrayType 			/*?*/arrayType = otherTypeDefinition as IArrayType;
			if (arrayType != null)
				return CanAccess(typeDefinition, arrayType.ElementType.ResolvedType);
			IGenericTypeInstance 			/*?*/genericTypeInstance = otherTypeDefinition as IGenericTypeInstance;
			if (genericTypeInstance != null) {
				if (!CanAccess(typeDefinition, genericTypeInstance.GenericType.ResolvedType))
					return false;
				foreach (var typeRef in genericTypeInstance.GenericArguments) {
					if (!CanAccess(typeDefinition, typeRef.ResolvedType))
						return false;
				}
				return true;
			}
			...
			if (genericParameter != null)
				return TypeHelper.TypesAreEquivalent(genericParameter.DefiningType, typeDefinition);
			return false;
		}
		...
		public static ITypeDefinition EffectiveBaseClass(IGenericParameter genericParameter)
		{
			...
			if (genericParameter.MustBeValueType) {
				...
				if (result is Dummy || !result.IsClass)
			...
			foreach (ITypeReference cref in genericParameter.Constraints) {
				...
				if (result is Dummy) {
					...
					Contract.Assert(bc == null || bc.IsClass);
					//Could be null if System.Object cannot be resolved
					Contract.Assert(result == Dummy.TypeDefinition || result.IsClass);
					if (bc != null) {
		...
		public static bool IsCompilerGenerated(ITypeDefinition type)
		{
			...
			while (true) {
				if (AttributeHelper.Contains(type.Attributes, type.PlatformType.SystemRuntimeCompilerServicesCompilerGeneratedAttribute))
					return true;
		...
		public static bool IsVisibleOutsideAssembly(ITypeDefinition typeDefinition)
		{
			...
			if (nestedType != null && !TypeHelper.IsVisibleOutsideAssembly(nestedType.ContainingTypeDefinition))
				return false;
		...
		public static bool IsVisibleToFriendAssemblies(ITypeDefinition typeDefinition)
		{
			...
			var nestedType = typeDefinition as INestedTypeDefinition;
			if (nestedType != null && !TypeHelper.IsVisibleToFriendAssemblies(nestedType.ContainingTypeDefinition))
				return false;
			switch (TypeHelper.TypeVisibilityAsTypeMemberVisibility(typeDefinition)) {
				case TypeMemberVisibility.Public:
				case TypeMemberVisibility.Family:
		...
		public static ITypeReference MergedType(ITypeReference type1, ITypeReference type2)
		{
			...
			if (TypesAreEquivalent(type1, type2))
				return type1;
			if (StackTypesAreEquivalent(type1, type2))
				return StackType(type1);
			...
			if (!(typeDef1 is Dummy)) {
				if (Type1ImplementsType2(typeDef1, type2))
					return type2;
			} else if (!(typeDef2 is Dummy)) {
				if (Type1ImplementsType2(typeDef2, type1))
					return type1;
			}
		...
		public static TypeMemberVisibility TypeVisibilityAsTypeMemberVisibility(ITypeDefinition type)
		{
			...
			TypeMemberVisibility result = TypeMemberVisibility.Public;
			// supposedly the only thing that doesn't meet any of the below tests are type parameters and their "default" is public.
		...
		public static ITypeReference UninstantiateAndUnspecialize(ITypeReference type)
		{
			...
			if (genericTypeInstance != null)
				return TypeHelper.UninstantiateAndUnspecialize(genericTypeInstance.GenericType);
		...
		public static TypeMemberVisibility VisibilityIntersection(TypeMemberVisibility visibility1, TypeMemberVisibility visibility2)
		{
			...
			switch (visibility1) {
				case TypeMemberVisibility.Assembly:
					switch (visibility2) {
						case TypeMemberVisibility.Assembly:
							result = TypeMemberVisibility.Assembly;
							break;
						case TypeMemberVisibility.Family:
							result = TypeMemberVisibility.FamilyAndAssembly;
							break;
						case TypeMemberVisibility.FamilyAndAssembly:
							result = TypeMemberVisibility.FamilyAndAssembly;
							break;
				...
				case TypeMemberVisibility.Family:
					switch (visibility2) {
						case TypeMemberVisibility.Assembly:
							...
							break;
						case TypeMemberVisibility.Family:
							result = TypeMemberVisibility.Family;
							break;
						case TypeMemberVisibility.FamilyAndAssembly:
							result = TypeMemberVisibility.FamilyAndAssembly;
							break;
				...
				case TypeMemberVisibility.FamilyOrAssembly:
					switch (visibility2) {
						...
						case TypeMemberVisibility.FamilyAndAssembly:
							...
							break;
						case TypeMemberVisibility.FamilyOrAssembly:
							result = TypeMemberVisibility.FamilyOrAssembly;
							break;
						case TypeMemberVisibility.Private:
							result = TypeMemberVisibility.Private;
							break;
		...
		public static IUnitNamespace GetDefiningNamespace(INamedTypeDefinition namedTypeDefinition)
		{
			...
			if (namespaceTypeDefinition != null)
				return namespaceTypeDefinition.ContainingUnitNamespace;
		...
		public static IUnit GetDefiningUnit(ITypeDefinition typeDefinition)
		{
			...
			if (genericTypeInstance != null)
				return TypeHelper.GetDefiningUnit(genericTypeInstance.GenericType.ResolvedType);
			IManagedPointerType 			/*?*/managedPointerType = typeDefinition as IManagedPointerType;
			if (managedPointerType != null)
				return TypeHelper.GetDefiningUnit(managedPointerType.TargetType.ResolvedType);
			IPointerType 			/*?*/pointerType = typeDefinition as IPointerType;
			if (pointerType != null)
				return TypeHelper.GetDefiningUnit(pointerType.TargetType.ResolvedType);
			IArrayType 			/*?*/arrayType = typeDefinition as IArrayType;
			if (arrayType != null)
				return TypeHelper.GetDefiningUnit(arrayType.ElementType.ResolvedType);
			IGenericTypeParameter 			/*?*/genericTypeParameter = typeDefinition as IGenericTypeParameter;
			if (genericTypeParameter != null)
				return TypeHelper.GetDefiningUnit(genericTypeParameter.DefiningType);
			IGenericMethodParameter 			/*?*/genericMethodParameter = typeDefinition as IGenericMethodParameter;
			if (genericMethodParameter != null)
				return TypeHelper.GetDefiningUnit(genericMethodParameter.DefiningMethod.ContainingType.ResolvedType);
		...
		public static IUnitReference GetDefiningUnitReference(ITypeReference typeReference)
		{
			...
			if (genericTypeInstanceReference != null)
				return TypeHelper.GetDefiningUnitReference(genericTypeInstanceReference.GenericType);
			...
			if (managedPointerTypeReference != null)
				return TypeHelper.GetDefiningUnitReference(managedPointerTypeReference.TargetType);
			IPointerTypeReference 			/*?*/pointerTypeReference = typeReference as IPointerTypeReference;
			if (pointerTypeReference != null)
				return TypeHelper.GetDefiningUnitReference(pointerTypeReference.TargetType);
			IArrayTypeReference 			/*?*/arrayTypeReference = typeReference as IArrayTypeReference;
			if (arrayTypeReference != null)
				return TypeHelper.GetDefiningUnitReference(arrayTypeReference.ElementType);
			...
			if (genericTypeParameterReference != null)
				return TypeHelper.GetDefiningUnitReference(genericTypeParameterReference.DefiningType);
			...
			if (genericMethodParameterReference != null)
				return TypeHelper.GetDefiningUnitReference(genericMethodParameterReference.DefiningMethod.ContainingType);
			Contract.Assume(false);
		...
		public static IFieldDefinition GetField(ITypeDefinition declaringType, IFieldReference fieldReference, bool resolveTypes = false)
		{
			...
			foreach (ITypeDefinitionMember member in declaringType.GetMembersNamed(fieldReference.Name, false)) {
				...
				if (field == null)
					continue;
				if (!TypeHelper.TypesAreEquivalent(field.Type, fieldReference.Type, resolveTypes))
					continue;
			...
			foreach (ITypeDefinitionMember member in declaringType.PrivateHelperMembers) {
				...
				if (field == null)
					continue;
				if (field.Name.UniqueKey != fieldReference.Name.UniqueKey)
					continue;
				if (!TypeHelper.TypesAreEquivalent(field.Type, fieldReference.Type, resolveTypes))
					continue;
				//TODO: check that custom modifiers are the same
		...
		public static IMethodDefinition GetMethod(ITypeDefinition declaringType, IMethodReference methodReference, bool resolveTypes = false)
		{
			...
			if (result is Dummy) {
				foreach (ITypeDefinitionMember member in declaringType.PrivateHelperMembers) {
					...
					if (meth == null)
						continue;
					if (meth.Name.UniqueKey != methodReference.Name.UniqueKey)
						continue;
					if (meth.GenericParameterCount != methodReference.GenericParameterCount)
						continue;
					if (meth.ParameterCount != methodReference.ParameterCount)
						continue;
					if (MemberHelper.SignaturesAreEqual(meth, methodReference, resolveTypes))
						return meth;
				}
			}
			return result;
		...
		public static IMethodDefinition GetInvokeMethod(ITypeDefinition delegateType, IMetadataHost host)
		{
			...
			return Dummy.MethodDefinition;
			//Should get here only when the delegate type is obtained from a malformed or malicious referenced assembly.
		}
		...
		public static IMethodDefinition GetMethod(IEnumerable<ITypeDefinitionMember> members, IMethodReference methodSignature, bool resolveTypes = false)
		{
			...
			foreach (ITypeDefinitionMember member in members) {
				...
				if (meth == null)
					continue;
				if (meth.GenericParameterCount != methodSignature.GenericParameterCount)
					continue;
				if (meth.ParameterCount != methodSignature.ParameterCount)
					continue;
				if (MemberHelper.SignaturesAreEqual(meth, methodSignature, resolveTypes))
					return meth;
			}
			return Dummy.MethodDefinition;
		}
		...
		/// </summary>
		[Pure()]
		public static ITypeDefinition MostDerivedCommonBaseClass(		/*?*/		//^ ensures result == null || result.IsClass;
ITypeDefinition type1, ITypeDefinition type2)
		{
			Contract.Requires(type1 != null);
			Contract.Requires(type2 != null);
			Contract.Ensures(Contract.Result<ITypeDefinition>() == null || Contract.Result<ITypeDefinition>().IsClass);
			...
			if (typeIter == null)
				return null;
			//type1 has no base classes
			if (TypeHelper.TypesAreEquivalent(typeIter, type2, true))
				return typeIter;
			//type1 is derived from type2.
			int depth1 = 0;
			while (typeIter != null) {
			...
			if (typeIter == null)
				return null;
			//type2 has no base classes
			if (TypeHelper.TypesAreEquivalent(typeIter, type1, true))
				return typeIter;
			//type2 is derived from type1.
			int depth2 = 0;
			while (typeIter != null) {
				typeIter = TypeHelper.BaseClass(typeIter);
				depth2++;
			...
			while (depth1 > depth2) {
				...
				Contract.Assume(type1 != null);
				//Because depth2 > 0 and we were able to call TypeHelper.BaseClass depth1 times before getting null.
				depth1--;
			}
			Contract.Assert(depth1 > 0);
			...
			while (depth2 > depth1) {
				...
				//Because depth1 > 0 and we were able to call TypeHelper.BaseClass depth2 times before getting null.
				depth2--;
			}
			Contract.Assert(depth2 > 0);
			...
			while (depth1 > 0) {
				...
				type1 = TypeHelper.BaseClass(type1);
				Contract.Assume(type1 != null);
				//Because depth1 > 0 and we were able to call TypeHelper.BaseClass depth1 times before getting null.
				type2 = TypeHelper.BaseClass(type2);
				Contract.Assume(type2 != null);
				//Because depth1 > 0 and we were able to call TypeHelper.BaseClass depth1 times before getting null.
				depth1--;
		...
		public static bool ParametersAreEquivalentAssumingGenericMethodParametersAreEquivalentIfTheirIndicesMatch(IParameterTypeInformation param1, IParameterTypeInformation param2, bool resolveTypes = false)
		{
			...
			if (param1.IsByReference != param2.IsByReference || param1.IsModified != param1.IsModified || !TypeHelper.TypesAreEquivalentAssumingGenericMethodParametersAreEquivalentIfTheirIndicesMatch(param1.Type, param2.Type, resolveTypes))
				return false;
		...
		public static uint SizeOfType(ITypeReference type, ITypeReference rootType, bool mayUseSizeOfProperty)
		{
			Contract.Requires(type != null);
			...
			switch (type.TypeCode) {
				...
				default:
					if (type.IsEnum && type.ResolvedType.IsEnum) {
						if (TypeHelper.TypesAreEquivalent(rootType, type.ResolvedType.UnderlyingType, true))
							return 0;
		...
		public static ushort TypeAlignment(ITypeReference type, ITypeReference rootType, bool mayUseAlignmentProperty)
		{
			Contract.Requires(type != null);
			Contract.Requires(rootType != null);
			Contract.Ensures(Contract.Result<ushort>() > 0);
			...
			/*?*/			/*?*/		public static bool ArrayTypesAreEquivalent(IArrayTypeReference arrayTypeRef1		, IArrayTypeReference arrayTypeRef2		, bool resolveTypes)
		{
			...
			if (!TypeHelper.TypesAreEquivalent(arrayTypeRef1.ElementType, arrayTypeRef2.ElementType, resolveTypes))
				return false;
			if (!IteratorHelper.EnumerablesAreEqual<ulong>(arrayTypeRef1.Sizes, arrayTypeRef2.Sizes) || !IteratorHelper.EnumerablesAreEqual<int>(arrayTypeRef1.LowerBounds, arrayTypeRef2.LowerBounds)) {
				return false;
			}
			return true;
		}
		...
		public static bool IsAttributeType(ITypeDefinition type)
		{
			...
			foreach (ITypeReference baseClass in type.BaseClasses) {
				if (TypeHelper.TypesAreEquivalent(baseClass, type.PlatformType.SystemAttribute, true))
					return true;
			}
			return false;
		}
		...
		/// </summary>
		[Pure()]
			/*?*/			/*?*/		public static bool FunctionPointerTypesAreEquivalentAssumingGenericMethodParametersAreEquivalentIfTheirIndicesMatch(IFunctionPointerTypeReference functionPointer1		, IFunctionPointerTypeReference functionPointer2		, bool resolveTypes = false)
		{
			if (functionPointer1 == null || functionPointer2 == null)
				return false;
			if (functionPointer1 == functionPointer2)
		...
		public static ITypeReference SpecializeTypeReference(ITypeReference typeReference, ITypeReference context, IInternFactory internFactory)
		{
			...
			if (arrayType != null) {
				if (arrayType.IsVector)
					return Vector.SpecializeTypeReference(arrayType, context, internFactory);
			...
			if (genericTypeParameter != null)
				return GenericParameter.SpecializeTypeReference(genericTypeParameter, context);
			var genericTypeInstance = typeReference as IGenericTypeInstanceReference;
			if (genericTypeInstance != null)
				return GenericTypeInstance.SpecializeTypeReference(genericTypeInstance, context, internFactory);
			var managedPointerType = typeReference as IManagedPointerTypeReference;
			if (managedPointerType != null)
				return ManagedPointerType.SpecializeTypeReference(managedPointerType, context, internFactory);
			var modifiedPointer = typeReference as ModifiedPointerType;
			if (modifiedPointer != null)
				return ModifiedPointerType.SpecializeTypeReference(modifiedPointer, context, internFactory);
			var modifiedType = typeReference as IModifiedTypeReference;
			if (modifiedType != null)
				return ModifiedTypeReference.SpecializeTypeReference(modifiedType, context, internFactory);
			var nestedType = typeReference as INestedTypeReference;
			if (nestedType != null)
				return SpecializedNestedTypeDefinition.SpecializeTypeReference(nestedType, context, internFactory);
			var pointerType = typeReference as IPointerTypeReference;
			if (pointerType != null)
				return PointerType.SpecializeTypeReference(pointerType, context, internFactory);
		...
		public static ITypeReference SpecializeTypeReference(ITypeReference typeReference, IMethodReference context, IInternFactory internFactory)
		{
			...
			if (genericMethodTypeParameter != null)
				return GenericParameter.SpecializeTypeReference(genericMethodTypeParameter, context, internFactory);
			var genericTypeInstance = typeReference as IGenericTypeInstanceReference;
			if (genericTypeInstance != null)
				return GenericTypeInstance.SpecializeTypeReference(genericTypeInstance, context, internFactory);
			var managedPointerType = typeReference as IManagedPointerTypeReference;
			...
			/*?*/			/*?*/		public static bool TypesAreEquivalentAssumingGenericMethodParametersAreEquivalentIfTheirIndicesMatch(ITypeReference type1		, ITypeReference type2		, bool resolveTypes = false)
		{
			...
			if (inst1 != null || inst2 != null) {
				if (inst1 == null || inst2 == null)
					return false;
				if (!TypeHelper.TypesAreEquivalentAssumingGenericMethodParametersAreEquivalentIfTheirIndicesMatch(inst1.GenericType, inst2.GenericType, resolveTypes))
					return false;
				return IteratorHelper.EnumerablesAreEqual<ITypeReference>(inst1.GenericArguments, inst2.GenericArguments, resolveTypes ? RelaxedTypeEquivalenceComparer.resolvingInstance : RelaxedTypeEquivalenceComparer.instance);
			}
		...
		/// </summary>
		public class RelaxedTypeEquivalenceComparer : IEqualityComparer<ITypeReference>
		{

			public RelaxedTypeEquivalenceComparer(bool resolveTypes = false)
			{
			...
			/// </summary>
			public static RelaxedTypeEquivalenceComparer instance = new RelaxedTypeEquivalenceComparer();

			/// <summary>
			/// A singleton instance of RelaxedTypeEquivalenceComparer that is safe to use in all contexts.
			/// </summary>
			public static RelaxedTypeEquivalenceComparer resolvingInstance = new RelaxedTypeEquivalenceComparer(true);
		...
		public static bool Type1DerivesFromOrIsTheSameAsType2(ITypeDefinition type1, ITypeReference type2, bool resolveTypes = false)
		{
			...
			if (TypeHelper.TypesAreEquivalent(type1, type2, resolveTypes))
				return true;
		...
		public static bool Type1DerivesFromType2(ITypeDefinition type1, ITypeReference type2, bool resolveTypes = false)
		{
			...
			foreach (ITypeReference baseClass in type1.BaseClasses) {
				if (TypeHelper.TypesAreEquivalent(baseClass, type2, resolveTypes))
					return true;
				if (TypeHelper.Type1DerivesFromType2(baseClass.ResolvedType, type2, resolveTypes))
					return true;
			}
			return false;
		}
		...
		public static bool Type1ImplementsType2(ITypeDefinition type1, ITypeReference type2, bool resolveTypes = false)
		{
			...
			foreach (ITypeReference implementedInterface in type1.Interfaces) {
				if (TypeHelper.TypesAreEquivalent(implementedInterface, type2, resolveTypes))
					return true;
				if (TypeHelper.Type1ImplementsType2(implementedInterface.ResolvedType, type2, resolveTypes))
					return true;
			}
			foreach (ITypeReference baseClass in type1.BaseClasses) {
				if (TypeHelper.Type1ImplementsType2(baseClass.ResolvedType, type2, resolveTypes))
					return true;
			}
			return false;
		}
		...
		public static bool Type1IsCovariantWithType2(ITypeDefinition type1, ITypeReference type2, bool resolveTypes = false)
		{
			...
			if (arrType1 == null || arrType2 == null)
				return false;
			if (arrType1.Rank != arrType2.Rank || arrType1.IsVector != arrType2.IsVector)
				return false;
		...
		public static bool TypesAreAssignmentCompatible(ITypeDefinition sourceType, ITypeDefinition targetType, bool resolveTypes = false)
		{
			...
			if (TypeHelper.TypesAreEquivalent(sourceType, targetType, resolveTypes))
				return true;
			if (sourceType.IsReferenceType && TypeHelper.Type1DerivesFromOrIsTheSameAsType2(sourceType, targetType, resolveTypes))
				return true;
			if (targetType.IsInterface && TypeHelper.Type1ImplementsType2(sourceType, targetType, resolveTypes))
				return true;
			if (sourceType.IsInterface && TypeHelper.TypesAreEquivalent(targetType, targetType.PlatformType.SystemObject, resolveTypes))
				return true;
			if (TypeHelper.Type1IsCovariantWithType2(sourceType, targetType, resolveTypes))
				return true;
			return false;
		}
		...
		public static ITypeReference UnsignedEquivalent(ITypeReference typeReference)
		{
			...
			switch (typeReference.TypeCode) {
				case PrimitiveTypeCode.Int8:
					return typeReference.PlatformType.SystemUInt8;
				case PrimitiveTypeCode.Int16:
					return typeReference.PlatformType.SystemUInt16;
				case PrimitiveTypeCode.Int32:
					return typeReference.PlatformType.SystemUInt32;
				case PrimitiveTypeCode.Int64:
					return typeReference.PlatformType.SystemUInt64;
				case PrimitiveTypeCode.IntPtr:
					return typeReference.PlatformType.SystemUIntPtr;
				default:
					return typeReference;
			}
		}
	...
	public class TypeNameFormatter
	{
		...
		public virtual string AddGenericParametersIfNeeded(ITypeReference type, ushort genericParameterCount, NameFormattingOptions formattingOptions, string typeName)
		{
			Contract.Requires(type != null);
			Contract.Requires(typeName != null);
			Contract.Ensures(Contract.Result<string>() != null);
			...
			if ((formattingOptions & NameFormattingOptions.TypeParameters) != 0 && (formattingOptions & NameFormattingOptions.FormattingForDocumentationId) == 0 && genericParameterCount > 0 && !(type.ResolvedType is Dummy)) {
				...
				if ((formattingOptions & NameFormattingOptions.TypeConstraints) != 0) {
					foreach (var parameter in type.ResolvedType.GenericParameters) {
						if (!parameter.MustBeReferenceType && !parameter.MustBeValueType && !parameter.MustHaveDefaultConstructor && IteratorHelper.EnumerableIsEmpty(parameter.Constraints))
							continue;
						sb.Append(" where ");
		...
		/// </summary>
		[Pure()]
		public virtual string GetArrayTypeName(IArrayTypeReference arrayType, NameFormattingOptions formattingOptions)
		{
		...
		/// </summary>
		public virtual void AppendArrayDimensions(IArrayTypeReference arrayType, StringBuilder sb, NameFormattingOptions formattingOptions)
		{
			Contract.Requires(arrayType != null);
			...
			if (formattingForDocumentationId && elementArrayType != null) {
				//Append the outer dimensions of the array first
			...
			sb.Append("]");
			if (!formattingForDocumentationId && elementArrayType != null) {
				//Append the inner dimensions of the array first
				this.AppendArrayDimensions(elementArrayType, sb, formattingOptions);
			}
		}
		...
		/// </summary>
		[Pure()]
		public virtual string GetGenericMethodParameterName(IGenericMethodParameterReference genericMethodParameter, NameFormattingOptions formattingOptions)
		{
		...
		/// </summary>
		[Pure()]
		public virtual string GetGenericTypeParameterName(IGenericTypeParameterReference genericTypeParameter, NameFormattingOptions formattingOptions)
		{
		...
		/// </summary>
		[Pure()]
		public virtual string GetManagedPointerTypeName(IManagedPointerTypeReference pointerType, NameFormattingOptions formattingOptions)
		{
		...
		/// </summary>
		public virtual string GetModifiedTypeName(IModifiedTypeReference modifiedType, NameFormattingOptions formattingOptions)
		{
		...
		/// </summary>
		[Pure()]
		public virtual string GetNamespaceTypeName(INamespaceTypeReference nsType, NameFormattingOptions formattingOptions)
		{
			Contract.Requires(nsType != null);
			Contract.Ensures(Contract.Result<string>() != null);

			var tname = nsType.Name.Value;
			if ((formattingOptions & NameFormattingOptions.EscapeKeyword) != 0)
				tname = this.EscapeKeyword(tname);
			if ((formattingOptions & NameFormattingOptions.SupressAttributeSuffix) != 0 && AttributeHelper.IsAttributeType(nsType.ResolvedType) & tname.EndsWith("Attribute", StringComparison.Ordinal))
			...
			if ((formattingOptions & NameFormattingOptions.DocumentationIdMemberKind) != 0)
				tname = "T:" + tname; else if ((formattingOptions & NameFormattingOptions.MemberKind) != 0)
		...
		/// </summary>
		[Pure()]
		public virtual string GetNestedTypeName(INestedTypeReference nestedType, NameFormattingOptions formattingOptions)
		{
			...
			if ((formattingOptions & NameFormattingOptions.EscapeKeyword) != 0)
				tname = this.EscapeKeyword(tname);
			if ((formattingOptions & NameFormattingOptions.SupressAttributeSuffix) != 0 && AttributeHelper.IsAttributeType(nestedType.ResolvedType) & tname.EndsWith("Attribute", StringComparison.Ordinal))
				tname = tname.Substring(0, tname.Length - 9);
		...
		/// </summary>
		[Pure()]
		public virtual string GetPointerTypeName(IPointerTypeReference pointerType, NameFormattingOptions formattingOptions)
		{
			Contract.Requires(pointerType != null);
			Contract.Ensures(Contract.Result<string>() != null);
		...
		/// </summary>
		[Pure()]
		public virtual string GetTypeKind(ITypeReference typeReference)
		{
			Contract.Requires(typeReference != null);
			Contract.Ensures(Contract.Result<string>() != null);
		...
		/// </summary>
		[Pure()]
		public virtual string GetFunctionPointerTypeName(IFunctionPointerTypeReference functionPointerType, NameFormattingOptions formattingOptions)
		{
		...
		/// </summary>
		[Pure()]
		public virtual string GetGenericTypeInstanceName(IGenericTypeInstanceReference genericTypeInstance, NameFormattingOptions formattingOptions)
		{
