...
namespace Microsoft.Cci.UtilityDataStructures
{
	...
	public static class HashHelper
	{
		...
		{
			unchecked {
				uint hash = 0xb1635d64u + key;
				hash += (hash << 3);
				hash ^= (hash >> 11);
				hash += (hash << 15);
				hash |= 0x1;
				//  To make sure that this is relatively prime with power of 2
				return hash;
			}
		}
		public static uint HashDoubleInt1(uint key1, uint key2)
		{
			unchecked {
				uint a = 0x9e3779b9u + key1;
				uint b = 0x9e3779b9u + key2;
				uint c = 16777619;
				a -= b;
				a -= c;
				a ^= (c >> 13);
		...
		public static uint HashDoubleInt2(uint key1, uint key2)
		{
			unchecked {
				uint hash = 0xb1635d64u + key1;
				hash += (hash << 10);
				hash ^= (hash >> 6);
				hash += key2;
				hash += (hash << 3);
	...
	public sealed class MultiHashtable<InternalT> where InternalT : class
	{
		...
		public static uint SizeFromExpectedEntries(uint expectedEntries)
		{
			uint expectedSize = (expectedEntries * 10) / 6;
			;
			uint initialSize = 16;
			while (initialSize < expectedSize && initialSize > 0)
				initialSize <<= 1;
			return initialSize;
		}
		...
		public void Expand()
		{
			...
			lock (this) {
				//force this.keyValueTable into memory before this.size gets increased.
				this.size <<= 1;
			}
			this.count = 0;
			this.resizeCount = this.size * 6 / 10;
		...
		public void AddInternal(uint key, InternalT value)
		{
			unchecked {
		...
		{
			public MultiHashtable<InternalT> MultiHashtable;
			public uint Key;
			public uint Hash1;
			public uint Hash2;
			public uint CurrentIndex;
			public KeyedValuesEnumerator(MultiHashtable<InternalT> multiHashtable, uint key)
			{
				this.MultiHashtable = multiHashtable;
				this.Key = key;
				this.Hash1 = HashHelper.HashInt1(key);
				this.Hash2 = HashHelper.HashInt2(key);
				this.CurrentIndex = 0xffffffffu;
			}
			...
			public InternalT Current {
				get { return this.MultiHashtable.keyValueTable[this.CurrentIndex].Value; }
			}
		...
		public struct KeyedValuesEnumerable
		{
			...
			public KeyedValuesEnumerable(MultiHashtable<InternalT> multiHashtable, uint key)
			{
				this.MultiHashtable = multiHashtable;
				this.Key = key;
			}
		...
		public struct ValuesEnumerator
		{
			...
			public ValuesEnumerator(MultiHashtable<InternalT> multiHashtable)
			{
				this.MultiHashtable = multiHashtable;
				this.CurrentIndex = 0xffffffffu;
			}
		...
		public struct ValuesEnumerable
		{
			...
			public ValuesEnumerable(MultiHashtable<InternalT> multiHashtable)
			{
				this.MultiHashtable = multiHashtable;
			}
	...
	public sealed class Hashtable<Key, Value> where Key : class where Value : class, new()
	{
		...
		public void AddInternal(Key key, Value value)
		{
			unchecked {
				uint mask = this.size - 1;
				var keyValueTable = this.keyValueTable;
	...
	public sealed class HashtableForUintValues<Key> where Key : class
	{
		...
		/// Constructor for Hashtable
		/// </summary>
		public HashtableForUintValues() : this(16)
		{
		}
		...
		public void AddInternal(Key key, uint value)
		{
			unchecked {
				uint mask = this.size - 1;
				var keyValueTable = this.keyValueTable;
	...
	public sealed class Hashtable<InternalT> where InternalT : class
	{
		...
		public struct ValuesEnumerator
		{
			public Hashtable<InternalT> Hashtable;
			public uint CurrentIndex;
			public ValuesEnumerator(Hashtable<InternalT> hashtable)
			{
			...
			/// </summary>
			public InternalT Current {
				get { return this.Hashtable.keyValueTable[this.CurrentIndex].Value; }
			}
		...
		public struct ValuesEnumerable
		{
			...
			public ValuesEnumerable(Hashtable<InternalT> hashtable)
			{
				this.Hashtable = hashtable;
			}
	...
	public sealed class Hashtable
	{
		...
		public void AddInternal(uint key, uint value)
		{
			unchecked {
				uint mask = this.size - 1;
				var keyValueTable = this.keyValueTable;
		...
		public struct ValuesEnumerator
		{
			...
			public uint CurrentIndex;
			public ValuesEnumerator(Hashtable hashtable)
			{
				this.Hashtable = hashtable;
				this.CurrentIndex = 0xffffffffu;
			}
		...
		public struct ValuesEnumerable
		{
			...
			public ValuesEnumerable(Hashtable hashtable)
			{
				this.Hashtable = hashtable;
			}
	...
	{
		public struct Key1Key2ValueTriple
		{
			public uint Key1;
			public uint Key2;
			public uint Value;
		...
		/// </summary>
		public DoubleHashtable() : this(16)
		{
		}
		...
		public void Expand()
		{
			...
			lock (this) {
				//force this.keysValueTable into memory before this.size gets increased.
				this.size <<= 1;
			}
			this.count = 0;
			this.resizeCount = this.size * 6 / 10;
		...
		public bool AddInternal(uint key1, uint key2, uint value)
		{
			unchecked {
				uint mask = this.size - 1;
	...
	public sealed class DoubleHashtable<T> where T : class
	{
		...
		public void Expand()
		{
			...
			this.keysValueTable = new Key1Key2ValueTriple[this.size * 2];
			lock (this) {
				//Force this.keysValueTable to get updated before anyone can read the new Size value
				this.size <<= 1;
			}
			this.count = 0;
			this.resizeCount = this.size * 6 / 10;
		...
		public bool AddInternal(uint key1, uint key2, T value)
		{
			unchecked {
				var keysValueTable = this.keysValueTable;
				uint hash1 = HashHelper.HashDoubleInt1(key1, key2);
	...
	public sealed class SetOfObjects
	{
		...
		public void Expand()
		{
			...
			lock (this) {
				//force this.elements into memory before this.size gets increased.
				this.size <<= 1;
			}
			this.count = 0;
			this.resizeCount = this.size * 6 / 10;
		...
		public bool AddInternal(object element)
		{
			unchecked {
				uint mask = this.size - 1;
		...
		public struct ValuesEnumerator
		{
			...
			public ValuesEnumerator(SetOfObjects setOfObjects)
			{
				this.setOfObjects = setOfObjects;
				this.currentIndex = 0xffffffffu;
			}

			public SetOfObjects setOfObjects;
			public uint currentIndex;
			...
			public object Current {
				get { return this.setOfObjects.elements[this.currentIndex]; }
			}
		...
		public struct ValuesEnumerable
		{
			public ValuesEnumerable(SetOfObjects setOfObjects)
			{
				this.setOfObjects = setOfObjects;
			}
	...
	public struct Sublist<T>
	{
		...
		public Sublist(List<T> masterList, int offset, int count)
		{
			...
			Contract.Assume(Contract.ForAll(masterList, e => e != null));
			//Too hard for clients to prove right now.
