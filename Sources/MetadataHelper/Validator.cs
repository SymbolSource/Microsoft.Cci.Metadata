...
namespace Microsoft.Cci
{
	...
	public class MetadataValidator
	{
		...
		public MetadataValidator(IMetadataHost host) : this(host, new ValidatingTraverser {
			PreorderVisitor = new ValidatingVisitor(),
		...
		public MetadataValidator(IMetadataHost host, ValidatingTraverser traverser)
		{
			this.host = host;
		...
		/// </summary>
		public IDefinition currentDefinition;
		...
		/// </summary>
		public ISecurityAttribute 		/*?*/currentSecurityAttribute;

		/// <summary>
		/// An instance of IDocument that corresponds to this.currentModule and that is becomes part of any error messages.
		/// </summary>
		public MetadataDocument document;

		/// <summary>
		/// An empty enumeration of related error locations.
		/// </summary>
		public static readonly IEnumerable<ILocation> emptyLocations = Enumerable<ILocation>.Empty;
		...
		/// </summary>
		public class ValidatingVisitor : IMetadataVisitor
		{

			/// <summary>
			/// The validator using this visitor.
			/// </summary>
			public MetadataValidator validator;

			/// <summary>
			/// tracks definitions 
			/// </summary>
			public readonly SetOfObjects definitionsAlreadyVisited = new SetOfObjects();

			/// <summary>
			/// 
			/// </summary>
			public readonly SetOfObjects allTypes = new SetOfObjects();
			...
			public void Visit(IAliasForType aliasForType)
			{
				...
				//this is an alias for an exported type, not a forwarded type.
				if (!(definingModule is IAssemblyReference) && !(definingModule.ContainingAssembly.AssemblyIdentity.Equals(this.validator.currentAssembly.AssemblyIdentity))) {
					this.ReportError(MetadataError.AliasedTypeDoesNotBelongToAModule, aliasForType, definingModule);
					return;
				}
				foreach (var aliasMember in aliasForType.Members) {
					//TODO: make the members element type be INestedAliasForType
			...
			public void Visit(IAssembly assembly)
			{
				...
				if ((assembly.Flags & ~(0x1 | 0x100 | 0x4000 | 0x8000 | 0x10 | 0x20 | 0x30 | 0x40 | 0x70 | 0x80)) != 0)
				...
				if (IteratorHelper.EnumerableIsEmpty(assembly.Files)) {
					foreach (var typeAlias in assembly.ExportedTypes) {
						//TODO: simplify this when type forwarders are put in their own collection.
			...
			public void CheckResourcesForUniqueness(IAssembly assembly)
			{
			...
			public void Visit(ICustomAttribute customAttribute)
			{
				if (customAttribute.Constructor is Dummy)
					this.ReportError(MetadataError.IncompleteNode, customAttribute, "Constructor"); else if (customAttribute.Type is Dummy)
					...
					if (!(customAttribute.Constructor.ResolvedMethod is Dummy)) {
						...
						if (!IteratorHelper.EnumerableHasLength(customAttribute.Arguments, customAttribute.Constructor.ResolvedMethod.ParameterCount)) {
							if (this.validator.currentSecurityAttribute == null || customAttribute.Constructor.ResolvedMethod.ParameterCount != 1 || IteratorHelper.EnumerableIsNotEmpty(customAttribute.Arguments))
			...
			public void Visit(ICustomModifier customModifier)
			{
				if (customModifier.Modifier is Dummy)
					this.ReportError(MetadataError.IncompleteNode, customModifier, "Modifier"); else if (!(customModifier.Modifier is INamedTypeReference))
			...
			public void Visit(IFieldDefinition fieldDefinition)
			{
				...
				if (fieldDefinition.IsMapped) {
					if (!fieldDefinition.Type.IsValueType)
						//TODO: also check, recursively, that all fields are public and have types that do not reference heap objects.
				...
				if (fieldDefinition.IsMarshalledExplicitly) {
					...
					if (fieldDefinition.MarshallingInformation.UnmanagedType == System.Runtime.InteropServices.UnmanagedType.LPArray || fieldDefinition.MarshallingInformation.UnmanagedType == System.Runtime.InteropServices.UnmanagedType.ByValArray || fieldDefinition.MarshallingInformation.UnmanagedType == System.Runtime.InteropServices.UnmanagedType.ByValTStr) {
			...
			public void CheckUnmanagedType(IMarshallingInformation marshallingInformation, uint type, string propertyName)
			{
				if (type < 0x2 || (0xc < type && type < 0x13 && type != 0xf) || type == 0x18 || type == 0x21 || type == 0x27 || type == 0x29 || (0x2d < type && type != 0x50)) {
			...
			public void Visit(IMetadataConstant constant)
			{
				...
				switch (ctype.TypeCode) {
					case PrimitiveTypeCode.Boolean:
						validValue = constant.Value is bool;
						break;
					case PrimitiveTypeCode.Char:
						validValue = constant.Value is char;
						break;
					case PrimitiveTypeCode.Int8:
						validValue = constant.Value is sbyte;
						break;
					case PrimitiveTypeCode.UInt8:
						validValue = constant.Value is byte;
						break;
					case PrimitiveTypeCode.Int16:
						validValue = constant.Value is short;
						break;
					case PrimitiveTypeCode.UInt16:
						validValue = constant.Value is ushort;
						break;
					case PrimitiveTypeCode.Int32:
						validValue = constant.Value is int;
						break;
					case PrimitiveTypeCode.UInt32:
						validValue = constant.Value is uint;
						break;
					case PrimitiveTypeCode.Int64:
						validValue = constant.Value is long;
						break;
					case PrimitiveTypeCode.UInt64:
						validValue = constant.Value is ulong;
						break;
					case PrimitiveTypeCode.Float32:
						validValue = constant.Value is float;
						break;
					case PrimitiveTypeCode.Float64:
						validValue = constant.Value is double;
						break;
			...
			public void Visit(IMethodDefinition method)
			{
				...
				if (method.HasDeclarativeSecurity) {
					if (IteratorHelper.EnumerableIsEmpty(method.SecurityAttributes) && !AttributeHelper.Contains(method.Attributes, this.validator.host.PlatformType.SystemSecuritySuppressUnmanagedCodeSecurityAttribute))
						this.ReportError(MetadataError.MethodMarkedAsHavingDeclarativeSecurityHasNoSecurityAttributes, method);
				} else {
					if (IteratorHelper.EnumerableIsNotEmpty(method.SecurityAttributes) || AttributeHelper.Contains(method.Attributes, this.validator.host.PlatformType.SystemSecuritySuppressUnmanagedCodeSecurityAttribute))
			...
			public void CheckGenericMethodTypeParameterUniqueness(IMethodDefinition methodDefinition)
			{
				int count = 0;
					...
					if (resolvedImplementedMethod.IsAccessCheckedOnOverride) {
						switch (resolvedImplementedMethod.Visibility) {
							...
							case TypeMemberVisibility.FamilyAndAssembly:
								...
								if (implementedUnit is Dummy)
									break;
								//Complaining might be a false positive. Wait until runtime.
								if (implementedUnit.UnitIdentity.Equals(this.validator.currentModule.ModuleIdentity))
									break;
								var implementingAssembly = this.validator.currentModule as IAssembly;
								var implementedAssembly = implementedUnit as IAssembly;
								if (implementingAssembly != null && implementedAssembly != null && UnitHelper.AssemblyOneAllowsAssemblyTwoToAccessItsInternals(implementedAssembly, implementingAssembly))
									break;
								goto default;
							default:
			...
			public void Visit(ITypeDefinition typeDefinition)
			{
				...
				if (typeDefinition.SizeOf > 0) {
					if (typeDefinition.Layout == LayoutKind.Auto)
						this.ReportError(MetadataError.AutoLayoutTypesCannotSpecifySize, typeDefinition); else if (typeDefinition.SizeOf >= 0x100000 && typeDefinition.IsStruct)
				...
				if (typeDefinition.Layout != LayoutKind.Auto) {
					...
					foreach (var baseClass in typeDefinition.BaseClasses)
						this.CheckLayoutConsistency(baseClass.ResolvedType, typeDefinition);
				}
			...
			public void CheckEventNameUniqueness(ITypeDefinition typeDefinition)
			{
				Hashtable eventNameTable = null;
				foreach (var ev in typeDefinition.Events) {
					var key = (uint)ev.Name.UniqueKey;
					if (eventNameTable == null)
						eventNameTable = new Hashtable();
			...
			public void CheckFieldUniqueness(ITypeDefinition typeDefinition)
			{
			...
			public void CheckInterfaceUniqueness(ITypeDefinition typeDefinition)
			{
			...
			public void CheckMethodUniqueness(ITypeDefinition typeDefinition)
			{
			...
			public void CheckTypeParameterUniqueness(ITypeDefinition typeDefinition)
			{
			...
			public void CheckLayoutConsistency(ITypeDefinition baseTypeDefinition, ITypeDefinition derivedTypeDefinition)
			{
				if (baseTypeDefinition.Layout == derivedTypeDefinition.Layout)
					return;
				if (baseTypeDefinition is Dummy || TypeHelper.TypesAreEquivalent(baseTypeDefinition, derivedTypeDefinition.PlatformType.SystemObject) || TypeHelper.TypesAreEquivalent(baseTypeDefinition, derivedTypeDefinition.PlatformType.SystemValueType))
					return;
				this.ReportError(MetadataError.DerivedTypeHasDifferentLayoutFromBaseType, derivedTypeDefinition, baseTypeDefinition);
			}

			static List<int> validAlignments = new List<int>(new int[] {
				1,
				2,
				4,
				8,
			...
			public void Visit(ITypeReference typeReference)
			{
				...
				if (resolvedType is Dummy) {
					...
					if (assemRef != null && assemRef.UnifiedAssemblyIdentity != assemRef.AssemblyIdentity) {
						...
						if (TypeHelper.GetTypeName(typeReference) == TypeHelper.GetTypeName(resolvedType))
							return;
					}
					//If we get here the type had better be an alias
					if (!(typeReference is IGenericTypeInstanceReference)) {
						if (!typeReference.IsAlias)
							this.ReportError(MetadataError.TypeReferenceResolvesToDifferentType, typeReference); else if (typeReference.AliasForType.AliasedType.ResolvedType.InternedKey != resolvedType.InternedKey)
			...
			public void ReportError(MetadataError error, object node, params object[] relatedNodes)
			{
				var message = new ErrorMessage {
					Error = error,
					ErrorReporter = this,
					Location = new MetadataNode {
						Document = this.validator.document,
						Node = node
					}
				};
				if (relatedNodes.Length > 0) {
					...
					foreach (var relatedNode in relatedNodes)
						relatedLocations.Add(new MetadataNode {
							Document = this.validator.document,
							Node = relatedNode
						});
			...
			/// <param name="relatedNodes">Any other nodes that relate to this error.</param>
			public void ReportError(MetadataError error, object node, string messageParameter, params object[] relatedNodes)
			{
				var message = new ErrorMessage {
					Error = error,
					ErrorReporter = this,
			...
			static List<string> validCultureNames = new List<string>(new string[] {
				"ar-SA",
				"ar-IQ",
				"ar-EG",
				"ar-LY",
		...
		/// </summary>
		public class ValidatingTraverser : MetadataTraverser
		{
		...
		public sealed class ErrorMessage : IErrorMessage
		{
			...
			public string Message {
				get {
					...
					if (localizedString == null)
						localizedString = messageKey; else if (this.MessageParameter != null)
		...
		/// </summary>
		public sealed class MetadataNode : ILocation
		{
			...
			public override string ToString()
			{
				...
				//TODO: something a bit more informative
			}
		}
		...
		/// </summary>
		public sealed class MetadataDocument : IDocument
		{
		...
		public void IntializeDocument(IModule moduleToValidate)
		{
