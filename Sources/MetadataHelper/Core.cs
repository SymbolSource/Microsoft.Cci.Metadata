...
namespace Microsoft.Cci
{
	...
	public abstract class MetadataHostEnvironment : IMetadataHost
	{
		...
		public MetadataHostEnvironment(INameTable nameTable, IInternFactory factory, byte pointerSize, IEnumerable<string> searchPaths, bool searchInGAC)
		{
		...
		/// </summary>
		public virtual AssemblyIdentity GetContractAssemblySymbolicIdentity()
		{
			if (this.unitCache.Count > 0) {
				...
				lock (GlobalLock.LockingObject) {
					foreach (IUnit unit in this.unitCache.Values) {
						...
						if (contractId.Name.Value.Length == 0)
							continue;
						if (result == null || result.Version < contractId.Version)
							result = contractId;
					}
				}
				if (result != null)
		...
		/// </summary>
		public virtual AssemblyIdentity GetCoreAssemblySymbolicIdentity()
		{
			...
			if (this.unitCache.Count > 0) {
				...
				lock (GlobalLock.LockingObject) {
					foreach (IUnit unit in this.unitCache.Values) {
						...
						if (coreId.Name.Value.Length == 0)
							continue;
						if (result == null || result.Version == dummyVersion || (result.Version < coreId.Version && coreId.Version != dummyVersion) || result.Version == coreId.Version && unit.UnitIdentity.Equals(coreId)) {
							result = coreId;
							referringUnit = unit;
						}
					}
				...
				if (result != null) {
					...
					if (result.Location.Length == 0) {
						...
						if (loc.Length > 0) {
							...
							if (myCore.Equals(result))
								return myCore;
							//myCore is the same as result, but also has a non null location.
						}
						//Now use host specific heuristics for finding the assembly.
						this.coreAssemblySymbolicIdentity = result;
						//in case ProbeAssemblyReference wants to know the core identity
		...
		/// </summary>
		public AssemblyIdentity GetSystemCoreAssemblySymbolicIdentity()
		{
		...
		{
			if (moduleIdentity.Location == null)
				return Dummy.Module;
			IUnit 			/*?*/unit;
			lock (GlobalLock.LockingObject) {
				this.unitCache.TryGetValue(moduleIdentity, out unit);
		...
		public IUnit LoadUnit(UnitIdentity unitIdentity)
		{
			...
			if (assemblyIdentity != null)
				return this.LoadAssembly(assemblyIdentity);
			ModuleIdentity 			/*?*/moduleIdentity = unitIdentity as ModuleIdentity;
			if (moduleIdentity != null)
				return this.LoadModule(moduleIdentity);
		...
		/// </summary>
		public virtual IPlatformType GetPlatformType()
		{
		...
		/// </summary>
		//^ ensures result == 4 || result == 8;
		public virtual byte GetTargetPlatformPointerSize()
		{
			lock (GlobalLock.LockingObject) {
				if (this.unitCache.Count > 0) {
					foreach (IUnit unit in this.unitCache.Values) {
		...
		/// </summary>
		public AssemblyIdentity Probe(		/*?*/string probeDir, AssemblyIdentity referencedAssembly)
		{
		...
		public void SynchronousReportErrors(object state		)
		{
		...
		/// </summary>
		public virtual bool PreserveILLocations {
			get { return false; }
		}
	...
	public abstract class MetadataReaderHost : MetadataHostEnvironment, IMetadataReaderHost, IDisposable
	{
		...
		/// </param>
		//^ requires pointerSize == 0 || pointerSize == 4 || pointerSize == 8;
		public MetadataReaderHost(INameTable nameTable, IInternFactory factory, byte pointerSize, IEnumerable<string> searchPaths, bool searchInGAC) : base(nameTable, factory, pointerSize, searchPaths, searchInGAC)
		{
		}
		...
		/// </summary>
		public List<IDisposable> disposableObjectAllocatedByThisHost = new List<IDisposable>();
		...
		public bool TryNextPermutation()
		{
			...
			if (this.currentWildGuesses != null) {
				...
				foreach (var key in keys) {
					...
					if (oldValue == 4)
						this.currentWildGuesses[key] = 1; else if (oldValue == 1)
						this.currentWildGuesses[key] = 2; else if (oldValue == 2)
			...
			if (allPermutationsHaveBeenTried && this.currentGoodGuesses != null) {
				if (this.currentWildGuesses == null)
					this.currentWildGuesses = new Dictionary<uint, byte>();
		...
		public void WinGuessingGame()
		{
			if (this.currentWildGuesses == null)
				return;
			if (this.successfulGuesses == null)
				this.successfulGuesses = new Dictionary<uint, byte>();
			foreach (var pair in this.currentWildGuesses)
				this.successfulGuesses[pair.Key] = pair.Value;
			this.currentWildGuesses = null;
			if (this.currentGoodGuesses == null)
				return;
		...
		public byte GuessUnderlyingTypeSizeOfUnresolvableReferenceToEnum(ITypeReference reference)
		{
			...
			if (this.successfulGuesses != null && this.successfulGuesses.TryGetValue(rkey, out guess)) {
				if (this.currentGoodGuesses == null)
					this.currentGoodGuesses = new Dictionary<uint, byte>();
	...
	public sealed class InternFactory : IInternFactory
	{
		...
		public sealed class AssemblyStore
		{
			...
			public uint InternedIdWithCount;
			public readonly uint RootNamespaceInternedId;
			public AssemblyStore(AssemblyIdentity assemblyIdentity, uint internedId, uint rootNamespaceInternedId)
			{
			...
			public uint InternedId {
				get { return this.InternedIdWithCount & 0xfffff000u; }
			}
		}

		public sealed class ModuleStore
		{
			public readonly ModuleIdentity ModuleIdentitity;
			public readonly uint InternedId;
			public readonly uint RootNamespaceInternedId;
			public ModuleStore(ModuleIdentity moduleIdentitity, uint internedId, uint rootNamespaceInternedId)
			{
		...
		public sealed class NamespaceTypeStore
		{
			public readonly uint ContainingNamespaceInternedId;
			public readonly uint GenericParameterCount;
			public readonly uint InternedId;
			public NamespaceTypeStore(uint containingNamespaceInternedId, uint genericParameterCount, uint internedId)
			{
		...
		public sealed class NestedTypeStore
		{
			public readonly uint ContainingTypeInternedId;
			public readonly uint GenericParameterCount;
			public readonly uint InternedId;
			public NestedTypeStore(uint containingTypeInternedId, uint genericParameterCount, uint internedId)
			{
		...
		public sealed class MatrixTypeStore
		{
			public readonly int Rank;
			public readonly int[] LowerBounds;
			public readonly ulong[] Sizes;
			public readonly uint InternedId;
			public MatrixTypeStore(int rank, int[] lowerBounds, ulong[] sizes, uint internedId)
			{
				this.Rank = rank;
		...
		public sealed class ParameterTypeStore
		{
			public readonly bool IsByReference;
			public readonly uint CustomModifiersInternId;
			public readonly uint InternedId;
			public ParameterTypeStore(bool isByReference, uint customModifiersInternId, uint internedId)
			{
		...
		public sealed class SignatureStore
		{
			...
			public readonly uint RequiredParameterListInternedId;
			public readonly uint ExtraParameterListInternedId;
			public readonly bool ReturnValueIsByRef;
			public readonly uint ReturnValueCustomModifiersListInteredId;
			public readonly uint ReturnTypeReferenceInternedId;
			public readonly uint GenericParameterCount;
			public readonly uint InternedId;
			public SignatureStore(CallingConvention callingConvention, uint requiredParameterListInteredId, uint extraParameterListInteredId, bool returnValueIsByRef, uint returnValueCustomModifiersListInteredId, uint returnTypeReferenceInteredId, uint genericParameterCount, uint internedId)
			{
		...
		IMethodReference CurrentMethodReference;
		//The method reference currently being interned
		...
		{
			this.CurrentAssemblyInternValue = 0x1000;
			this.CurrentMethodReference = Dummy.MethodReference;
			this.CurrentModuleInternValue = 0x1;
			this.CurrentNamespaceInternValue = 0x1;
			this.CurrentTypeInternValue = 0x100;
			this.CurrentTypeListInternValue = 0x1;
			this.CurrentCustomModifierInternValue = 0x1;
			this.CurrentCustomModifierListInternValue = 0x1;
			this.CurrentMethodReferenceInternValue = 0x1;
			this.CurrentFieldReferenceInternValue = 0x1;
			this.CurrentParameterTypeInternValue = 0x1;
			this.CurrentParameterTypeListInternValue = 0x1;
			this.CurrentSignatureInternValue = 0x1;
		...
		public AssemblyStore GetAssemblyStore(AssemblyIdentity assemblyIdentity)
		{
		...
		public ModuleStore GetModuleStore(ModuleIdentity moduleIdentity)
		{
		...
		public uint GetUnitRootNamespaceInternId(IUnitReference unitReference)
		{
		...
		public uint GetNestedTypeReferenceInternId(ITypeReference containingTypeReference, IName typeName, uint genericParameterCount)
		{
			...
			foreach (NestedTypeStore nstTypeStore in this.NestedTypeHashtable.GetValuesFor((uint)typeName.UniqueKey)) {
				if (nstTypeStore.ContainingTypeInternedId == containingTypeReferenceInteredId && nstTypeStore.GenericParameterCount == genericParameterCount) {
		...
		public uint GetVectorTypeReferenceInternId(ITypeReference elementTypeReference)
		{
		...
		public uint GetMatrixTypeReferenceInternId(ITypeReference elementTypeReference, int rank, IEnumerable<ulong> sizes, IEnumerable<int> lowerBounds)
		{
			uint elementTypeReferenceInternId = this.GetTypeReferenceInternId(elementTypeReference);
		...
		public uint GetTypeReferenceListInternedId(IEnumerator<ITypeReference> typeReferences)
		{
		...
		public uint GetGenericTypeInstanceReferenceInternId(ITypeReference genericTypeReference, IEnumerable<ITypeReference> genericArguments)
		{
		...
		public uint GetPointerTypeReferenceInternId(ITypeReference targetTypeReference)
		{
		...
		public uint GetManagedPointerTypeReferenceInternId(ITypeReference targetTypeReference)
		{
			uint targetTypeReferenceInternId = this.GetTypeReferenceInternId(targetTypeReference);
		...
		public uint GetGenericTypeParameterReferenceInternId(ITypeReference definingTypeReference, int index)
		{
		...
		public uint GetGenericMethodParameterReferenceInternId(IMethodReference definingMethodReference, uint index)
		{
			if (this.CurrentMethodReference != Dummy.MethodReference) {
				...
				//provide a big offset to minimize the chances of a structural type in the 
				//signature of the method interning onto some other type that is parameterized by a type whose intern key is index.
			}
			this.CurrentMethodReference = definingMethodReference;
			//short circuit recursive calls back to this method
		...
		public uint GetParameterTypeInternId(IParameterTypeInformation parameterTypeInformation)
		{
			...
			foreach (ParameterTypeStore parameterTypeStore in this.ParameterTypeHashtable.GetValuesFor(typeReferenceInternId)) {
				if (parameterTypeStore.IsByReference == parameterTypeInformation.IsByReference && parameterTypeStore.CustomModifiersInternId == customModifiersInternId) {
		...
		public uint GetParameterTypeListInternId(IEnumerator<IParameterTypeInformation> parameterTypeInformations)
		{
		...
		public uint GetSignatureInternId(CallingConvention callingConvention, IEnumerable<IParameterTypeInformation> parameters, IEnumerable<IParameterTypeInformation> extraArgumentTypes, IEnumerable<ICustomModifier> returnValueCustomModifiers, bool returnValueIsByRef, ITypeReference returnType)
		{
			...
			foreach (SignatureStore signatureStore in this.SignatureHashtable.GetValuesFor(requiredParameterTypesInternedId)) {
				if (signatureStore.CallingConvention == callingConvention && signatureStore.RequiredParameterListInternedId == requiredParameterTypesInternedId && signatureStore.ExtraParameterListInternedId == extraArgumentTypesInteredId && signatureStore.ReturnValueCustomModifiersListInteredId == returnValueCustomModifiersInternedId && signatureStore.ReturnValueIsByRef == returnValueIsByRef && signatureStore.ReturnTypeReferenceInternedId == returnTypeReferenceInternedId) {
		...
		public uint GetMethodReferenceInternedId(IMethodReference methodReference)
		{
			var genInstanceRef = methodReference as IGenericMethodInstanceReference;
			if (genInstanceRef != null)
				return this.GetGenericMethodInstanceReferenceInternedKey(genInstanceRef);
			...
			foreach (SignatureStore signatureStore in methods.GetValuesFor((uint)methodReference.Name.UniqueKey)) {
				if (signatureStore.CallingConvention == methodReference.CallingConvention && signatureStore.RequiredParameterListInternedId == requiredParameterTypesInternedId && signatureStore.ReturnValueCustomModifiersListInteredId == returnValueCustomModifiersInternedId && signatureStore.ReturnValueIsByRef == methodReference.ReturnValueIsByRef && signatureStore.ReturnTypeReferenceInternedId == returnTypeReferenceInternedId && signatureStore.GenericParameterCount == genericParameterCount) {
					return signatureStore.InternedId;
				}
			}
			SignatureStore signatureStore1 = new SignatureStore(methodReference.CallingConvention, requiredParameterTypesInternedId, 0, methodReference.ReturnValueIsByRef, returnValueCustomModifiersInternedId, returnTypeReferenceInternedId, genericParameterCount, this.CurrentMethodReferenceInternValue++);
			methods.Add((uint)methodReference.Name.UniqueKey, signatureStore1);
			return signatureStore1.InternedId;
		}

		public uint GetGenericMethodInstanceReferenceInternedKey(IGenericMethodInstanceReference genericMethodInstanceReference)
		{
		...
		public uint GetFieldReferenceInternedId(IFieldReference fieldReference)
		{
		...
		public uint GetFunctionPointerTypeReferenceInternId(CallingConvention callingConvention, IEnumerable<IParameterTypeInformation> parameters, IEnumerable<IParameterTypeInformation> extraArgumentTypes, IEnumerable<ICustomModifier> returnValueCustomModifiers, bool returnValueIsByRef, ITypeReference returnType)
		{
		...
		public uint GetCustomModifierInternId(ICustomModifier customModifier)
		{
			uint currentTypeRefInternedId = this.GetTypeReferenceInternId(customModifier.Modifier);
			uint isOptionalIntneredId = customModifier.IsOptional ? 0xf0f0f0f0u : 0xf0f0f0f;
		...
		public uint GetCustomModifierListInternId(IEnumerator<ICustomModifier> customModifiers)
		{
		...
		public uint GetTypeReferenceInterendIdIgnoringCustomModifiers(ITypeReference typeReference)
		{
			...
			INestedTypeReference 			/*?*/nestedTypeReference = typeReference as INestedTypeReference;
			if (nestedTypeReference != null) {
				return this.GetNestedTypeReferenceInternId(nestedTypeReference.ContainingType, nestedTypeReference.Name, nestedTypeReference.GenericParameterCount);
			}
			...
			if (arrayTypeReference != null) {
				if (arrayTypeReference.IsVector) {
					...
					return this.GetMatrixTypeReferenceInternId(arrayTypeReference.ElementType, (int)arrayTypeReference.Rank, arrayTypeReference.Sizes, arrayTypeReference.LowerBounds);
				}
			}
			IGenericTypeInstanceReference 			/*?*/genericTypeInstanceReference = typeReference as IGenericTypeInstanceReference;
			if (genericTypeInstanceReference != null) {
				return this.GetGenericTypeInstanceReferenceInternId(genericTypeInstanceReference.GenericType, genericTypeInstanceReference.GenericArguments);
			}
			...
			if (genericTypeParameterReference != null) {
				return this.GetGenericTypeParameterReferenceInternId(genericTypeParameterReference.DefiningType, (int)genericTypeParameterReference.Index);
			}
			...
			if (functionPointerTypeReference != null) {
				...
				return this.GetFunctionPointerTypeReferenceInternId(functionPointerTypeReference.CallingConvention, functionPointerTypeReference.Parameters, functionPointerTypeReference.ExtraArgumentTypes, returnValueCustomModifiers, functionPointerTypeReference.ReturnValueIsByRef, functionPointerTypeReference.Type);
			}
		...
		public uint GetModifiedTypeReferenceInternId(ITypeReference typeReference, IEnumerable<ICustomModifier> customModifiers)
		{
	...
	public sealed class NameTable : INameTable
	{
		...
		int caseInsensitiveCounter = 1;
		//^ invariant caseInsensitiveCounter >= 0;
		int caseSensitiveCounter = 3;
		//^ invariant caseSensitiveCounter >= 0;
		/// <summary>
		...
		{
			string value;
			int caseInsensitiveUniqueKey;
			//^ invariant caseInsensitiveUniqueKey > 0;
			int uniqueKey;
			//^ invariant uniqueKey > 0;
			//^ requires caseInsensitiveUniqueKey > 0 && uniqueKey > 0;
			public Name(int caseInsensitiveUniqueKey, int uniqueKey, string value)
			{
