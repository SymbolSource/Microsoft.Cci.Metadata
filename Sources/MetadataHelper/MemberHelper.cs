...
namespace Microsoft.Cci
{
	...
	public static class MemberHelper
	{
			...
			foreach (ITypeDefinitionMember member in members) {
				...
				if (fieldAsTypeDef != null && fieldAsTypeDef == item) {
					...
					if (f.Type.ResolvedType == item)
						continue;
					// in case we are calculating the offset of an anonymous type, skip the implicit field of that type
		...
		public static uint GetFieldOffset(IFieldDefinition field)
		{
			...
			if (typeDefinition.Layout == LayoutKind.Explicit)
				return field.Offset;
			return ComputeFieldOffset(field, field.ContainingTypeDefinition);
			// TODO use typeDefinition
		}
		...
		public static IEnumerable<IMethodDefinition> GetImplicitlyImplementedInterfaceMethods(IMethodDefinition implementingMethod)
		{
			...
			if (!implementingMethod.IsVirtual)
				yield break;
			if (implementingMethod.Visibility != TypeMemberVisibility.Public)
				yield break;
			if (implementingMethod.ContainingTypeDefinition.IsInterface)
				yield break;
		...
		public static bool IsVisibleOutsideAssembly(ITypeDefinitionMember typeDefinitionMember)
		{
			...
			if (!TypeHelper.IsVisibleOutsideAssembly(containingTypeDefinition))
				return false;
		...
		/// <param name="containingTypeDefinition">The type definition that contains the type member that is or contains the method reference.</param>
			/*?*/		public static bool IsExplicitImplementationVisible(IMethodReference methodReference		, ITypeDefinition containingTypeDefinition)
		{
			Contract.Requires(containingTypeDefinition != null);
			...
			foreach (IMethodImplementation methodImpl in containingTypeDefinition.ExplicitImplementationOverrides) {
				...
				if (IsVisibleOutsideAssembly(implementedMethod))
					return true;
			}
			return false;
		}
		...
		public static bool IsVolatile(IFieldDefinition field)
		{
			...
			foreach (ICustomModifier customModifier in field.CustomModifiers) {
				if (customModifier.Modifier.InternedKey == isVolatileKey)
					return true;
			}
			return false;
		}
		...
		public static bool SignaturesAreEqual(ISignature signature1, ISignature signature2, bool resolveTypes = false)
		{
			...
			if (signature1.ReturnValueIsModified != signature2.ReturnValueIsModified)
				return false;
			if (!TypeHelper.TypesAreEquivalent(signature1.Type, signature2.Type, resolveTypes))
				return false;
			return IteratorHelper.EnumerablesAreEqual(signature1.Parameters, signature2.Parameters, resolveTypes ? ResolvingParameterInformationComparer : ParameterInformationComparer);
		}
		...
		public static bool GenericMethodSignaturesAreEqual(ISignature method1, ISignature method2, bool resolveTypes = false)
		{
			...
			if (method1.CallingConvention != method2.CallingConvention)
				return false;
			if (method1.ReturnValueIsByRef != method2.ReturnValueIsByRef)
				return false;
			if (method1.ReturnValueIsModified != method2.ReturnValueIsModified)
				return false;
			if (!TypeHelper.TypesAreEquivalentAssumingGenericMethodParametersAreEquivalentIfTheirIndicesMatch(method1.Type, method2.Type, resolveTypes))
				return false;
			return IteratorHelper.EnumerablesAreEqual(method1.Parameters, method2.Parameters, resolveTypes ? ResolvingGenericMethodParameterEqualityComparer : GenericMethodParameterEqualityComparer);
		}

		/// <summary>
		/// A static instance of type GenericMethodParameterInformationComparer.
		/// </summary>
		public static readonly GenericMethodParameterInformationComparer GenericMethodParameterEqualityComparer = new GenericMethodParameterInformationComparer();

		/// <summary>
		/// A static instance of type GenericMethodParameterInformationComparer, which will resolve types if necessary.
		/// </summary>
		public static readonly GenericMethodParameterInformationComparer ResolvingGenericMethodParameterEqualityComparer = new GenericMethodParameterInformationComparer(true);

		/// <summary>
		/// A static instance of type ParameterInformationComparer.
		/// </summary>
		public static readonly ParameterInformationComparer ParameterInformationComparer = new ParameterInformationComparer();
	...
	public class MethodReference : IMethodReference
	{
		...
		public MethodReference(IMetadataHost host, ITypeReference containingType, CallingConvention callingConvention, ITypeReference returnType, IName name, ushort genericParameterCount, params ITypeReference[] parameterTypes)
		{
		...
		public MethodReference(IMetadataHost host, ITypeReference containingType, CallingConvention callingConvention, ITypeReference returnType, IName name, ushort genericParameterCount, IEnumerable<IParameterTypeInformation> parameters, params ITypeReference[] extraParameterTypes)
		{
			Contract.Requires(host != null);
			Contract.Requires(containingType != null);
			Contract.Requires(returnType != null);
...
//^^ ensures this.IsGeneric ==> result > 0;
	...
	public class GenericMethodParameterInformationComparer : IEqualityComparer<IParameterTypeInformation>
	{
		...
		{
			if (parameterTypeInformation == null)
				return 0;
	...
	public class SignatureFormatter
	{
		...
		/// </summary>
		public readonly TypeNameFormatter typeNameFormatter;
		...
		/// methods, the formatting can be customized for other languages.
		/// </summary>
		public SignatureFormatter() : this(new TypeNameFormatter())
		{
		}
		...
		public virtual string GetEventSignature(IEventDefinition eventDef, NameFormattingOptions formattingOptions)
		{
			...
			if ((formattingOptions & NameFormattingOptions.EscapeKeyword) != 0)
				eventName = this.typeNameFormatter.EscapeKeyword(eventName);
			if ((formattingOptions & NameFormattingOptions.OmitImplementedInterface) != 0) {
				int dotPos = eventName.IndexOf('.');
				if (dotPos > 0 && dotPos < eventName.Length - 1)
					eventName = eventName.Substring(dotPos + 1, eventName.Length - dotPos - 1);
			}
		...
		public virtual string GetFieldSignature(IFieldReference field, NameFormattingOptions formattingOptions)
		{
			...
			if ((formattingOptions & NameFormattingOptions.DocumentationIdMemberKind) != 0)
				sb.Append("F:"); else if ((formattingOptions & NameFormattingOptions.MemberKind) != 0)
				sb.Append("field ");
			if ((formattingOptions & NameFormattingOptions.OmitContainingType) == 0) {
			...
			if ((formattingOptions & NameFormattingOptions.EscapeKeyword) != 0)
				fieldName = this.typeNameFormatter.EscapeKeyword(fieldName);
			if ((formattingOptions & NameFormattingOptions.OmitImplementedInterface) != 0) {
				int dotPos = fieldName.IndexOf('.');
				if (dotPos > 0 && dotPos < fieldName.Length - 1)
					fieldName = fieldName.Substring(dotPos + 1, fieldName.Length - dotPos - 1);
			}
			if ((formattingOptions & NameFormattingOptions.FormattingForDocumentationId) != 0) {
		...
		public virtual string GetMemberSignature(ITypeMemberReference member, NameFormattingOptions formattingOptions)
		{
			...
			if (type != null)
				return this.typeNameFormatter.GetTypeName(type, formattingOptions);
			...
			if ((formattingOptions & NameFormattingOptions.FormattingForDocumentationId) != 0)
				name = this.MapToDocumentationIdName(name);
			return name;
		}
		...
		public virtual string GetMethodSignature(IMethodReference method, NameFormattingOptions formattingOptions)
		{
			...
			if ((formattingOptions & NameFormattingOptions.Modifiers) != 0) {
				if (method.IsStatic)
					sb.Append("static ");
				if (method.ResolvedMethod.IsAbstract)
					sb.Append("abstract ");
				if (method.ResolvedMethod.IsExternal)
					sb.Append("external ");
				if (method.ResolvedMethod.IsNewSlot)
					sb.Append("new ");
				if (method.ResolvedMethod.IsSealed)
					sb.Append("sealed ");
				if (method.ResolvedMethod.IsVirtual)
					sb.Append("virtual ");
			}
			if ((formattingOptions & NameFormattingOptions.Visibility) != 0) {
			...
			if (genericMethodInstance != null)
				this.AppendGenericArguments(genericMethodInstance, formattingOptions, sb); else if (method.IsGeneric)
		...
		/// </summary>
		public virtual void AppendGenericArguments(IGenericMethodInstanceReference method, NameFormattingOptions formattingOptions, StringBuilder sb)
		{
			Contract.Requires(method != null);
		...
		/// </summary>
		public virtual void AppendGenericParameters(IMethodReference method, NameFormattingOptions formattingOptions, StringBuilder sb)
		{
			Contract.Requires(method != null);
			Contract.Requires(sb != null);
		...
		/// </summary>
		public virtual void AppendMethodParameters(IMethodReference method, NameFormattingOptions formattingOptions, StringBuilder sb)
		{
			Contract.Requires(method != null);
			Contract.Requires(sb != null);

			var parameters = method.Parameters;
			if ((formattingOptions & NameFormattingOptions.Signature) == 0 || ((formattingOptions & NameFormattingOptions.FormattingForDocumentationId) != 0 && !IteratorHelper.EnumerableIsNotEmpty<IParameterTypeInformation>(parameters)))
				return;
			sb.Append('(');
			bool first = true;
			string delim = ((formattingOptions & NameFormattingOptions.OmitWhiteSpaceAfterListDelimiter) == 0) ? ", " : ",";
		...
		/// </summary>
		public virtual void AppendMethodName(IMethodReference method, NameFormattingOptions formattingOptions, StringBuilder sb)
		{
			Contract.Requires(method != null);
			Contract.Requires(sb != null);

			if ((formattingOptions & NameFormattingOptions.OmitContainingType) == 0) {
				sb.Append(this.typeNameFormatter.GetTypeName(method.ContainingType, formattingOptions & ~(NameFormattingOptions.MemberKind | NameFormattingOptions.DocumentationIdMemberKind | NameFormattingOptions.TypeConstraints)));
				sb.Append('.');
			}
			...
			if ((formattingOptions & NameFormattingOptions.OmitImplementedInterface) != 0) {
				...
				if (dotPos > 0 && dotPos < methodName.Length - 1)
					methodName = methodName.Substring(dotPos + 1, methodName.Length - dotPos - 1);
			}
			if ((formattingOptions & NameFormattingOptions.EscapeKeyword) != 0)
				methodName = this.typeNameFormatter.EscapeKeyword(methodName);
			if ((formattingOptions & NameFormattingOptions.FormattingForDocumentationId) != 0)
				methodName = this.MapToDocumentationIdName(methodName);
		...
		/// </summary>
		public virtual void AppendParameter(IParameterTypeInformation param, NameFormattingOptions formattingOptions, StringBuilder sb)
		{
			...
			if ((formattingOptions & NameFormattingOptions.ParameterModifiers) != 0) {
				if (def != null) {
					if (def.IsOut)
						sb.Append("out "); else if (def.IsParameterArray)
						sb.Append("params "); else if (def.IsByReference)
						sb.Append("ref ");
				} else {
		...
		/// </summary>
		public virtual void AppendPropertyName(IPropertyDefinition property, NameFormattingOptions formattingOptions, StringBuilder sb)
		{
			Contract.Requires(property != null);
			Contract.Requires(sb != null);
			...
			if ((formattingOptions & NameFormattingOptions.DocumentationIdMemberKind) != 0)
				sb.Append("P:"); else if ((formattingOptions & NameFormattingOptions.MemberKind) != 0)
				sb.Append("property ");
			if ((formattingOptions & NameFormattingOptions.OmitContainingType) == 0) {
			...
			if ((formattingOptions & NameFormattingOptions.EscapeKeyword) != 0)
				propertyName = this.typeNameFormatter.EscapeKeyword(propertyName);
			if ((formattingOptions & NameFormattingOptions.OmitImplementedInterface) != 0) {
				int dotPos = propertyName.IndexOf('.');
				if (dotPos > 0 && dotPos < propertyName.Length - 1)
			...
			if ((formattingOptions & NameFormattingOptions.PreserveSpecialNames) == 0) {
				foreach (var attribute in property.ContainingTypeDefinition.Attributes) {
					if (!IteratorHelper.EnumerableHasLength(attribute.Arguments, 1))
						continue;
					var mdConst = IteratorHelper.First(attribute.Arguments) as IMetadataConstant;
					if (mdConst == null || mdConst.Value == null || !mdConst.Value.Equals(propertyName))
						continue;
					var nsType = attribute.Type as INamespaceTypeReference;
					if (nsType == null)
						continue;
					if (nsType.Name.Value != "DefaultMemberAttribute")
						continue;
					var ns = nsType.ContainingUnitNamespace as INestedUnitNamespaceReference;
					if (ns == null)
						continue;
					if (ns.Name.Value != "Reflection")
						continue;
					propertyName = "this";
					break;
				}
		...
		/// </summary>
		public virtual void AppendPropertyParameters(IEnumerable<IParameterDefinition> parameters, NameFormattingOptions formattingOptions, StringBuilder sb)
		{
			Contract.Requires(parameters != null);
			Contract.Requires(Contract.ForAll(parameters, x => x != null));
			Contract.Requires(sb != null);
			...
			if (isNotEmpty)
				sb.Append((formattingOptions & NameFormattingOptions.FormattingForDocumentationId) != 0 ? '(' : '[');
			bool first = true;
		...
		/// </summary>
		public virtual void AppendReturnTypeSignature(ISignature sig, NameFormattingOptions formattingOptions, StringBuilder sb)
		{
		...
		/// </summary>
		public virtual string MapToDocumentationIdName(string name)
		{
			Contract.Requires(name != null);
			Contract.Ensures(Contract.Result<string>() != null);
