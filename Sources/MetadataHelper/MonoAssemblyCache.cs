...
public class MonoAssemblyCache : AssemblyName.IAssemblyCache
{
	public MonoAssemblyCache()
	{
	}

	public int UninstallAssembly(uint dwFlags, 	[MarshalAs(UnmanagedType.LPWStr)]
string pszAssemblyName, IntPtr pvReserved, int pulDisposition)
	...
	public int QueryAssemblyInfo(uint dwFlags, 	[MarshalAs(UnmanagedType.LPWStr)]
string pszAssemblyName, ref AssemblyName.ASSEMBLY_INFO pAsmInfo)
	{
		pAsmInfo = new AssemblyName.ASSEMBLY_INFO();
		pAsmInfo.cbAssemblyInfo = 1;
		// just needs to be nonzero for our purposes
	...
	public int CreateAssemblyCacheItem(uint dwFlags, IntPtr pvReserved, out object ppAsmItem, 	[MarshalAs(UnmanagedType.LPWStr)]
string pszAssemblyName)
	...
	public int InstallAssembly(uint dwFlags, 	[MarshalAs(UnmanagedType.LPWStr)]
...
public class MonoAssemblyName : IAssemblyName
{
	...
	public int GetProperty(uint PropertyId, IntPtr pvProperty, ref uint pcbProperty)
	{
		...
		if (PropertyId == MY_ASM_NAME.NAME)
			sSource = m_sAssemblyName; else if (PropertyId == MY_ASM_NAME.CULTURE)
		// The Visual Studio version gets an empty string for culture.. otherwise it'd be m_sCulture.
			sSource = ""; else if (PropertyId == MY_ASM_NAME.CODEBASE_URL)
			sSource = "";
		...
		if (PropertyId == MY_ASM_NAME.MAJOR_VERSION || PropertyId == MY_ASM_NAME.MINOR_VERSION || PropertyId == MY_ASM_NAME.BUILD_NUMBER || PropertyId == MY_ASM_NAME.REVISION_NUMBER) {
			...
			string sValuePart = (PropertyId == MY_ASM_NAME.MAJOR_VERSION) ? versionParts[0] : ((PropertyId == MY_ASM_NAME.MINOR_VERSION) ? versionParts[1] : ((PropertyId == MY_ASM_NAME.BUILD_NUMBER) ? versionParts[2] : versionParts[3]));
	...
	{
		pccDisplayName = 1;
		// They only care if this is nonzero.
	...
	public int BindToObject(object refIID, object pAsmBindSink, IApplicationContext pApplicationContext, 	[MarshalAs(UnmanagedType.LPWStr)]
...
public class MonoAssemblyEnum : IAssemblyEnum
{
